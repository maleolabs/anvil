...
# Registry index is provisioned via `anvil standard sync <id>` (or
# `anvil standard sync --all`) — no manual curl or hand-edit needed.
# Trust anchors ship embedded for first-party standards; `anvil standard
# trust add` manages additional publishers (fail-closed per ADR-022).
...
**AI agent skills:** `anvil skill list` · `anvil skill install <name>` · `anvil skill update <name>` · `anvil skill uninstall <name>` — the single core-gated surface for installing and managing AI agent skills (Agent Skills, agentskills.io) for the coding agents you use with Anvil. Two sources flow through one gate: **core skills** ship inside the Anvil binary (lockstep with the CLI version, installable offline) and **standard skills** are per-skill release assets of an installed delivery lifecycle standard, installed through the full adoption pipeline (lifecycle + compatibility gates, trust anchors before the fetch, https-only fetch, fail-closed attested-digest verification, strict extraction — ADR-037 D4). Targets: `--agent` (default: auto-detect from the agent config folders on this machine; `all` = every supported agent: `claude-code | opencode | codex | gemini | cursor | zed | windsurf | cline`), `--scope repo|global` (repo = the current Anvil project's git root, requires an Anvil project; global = your home-level agent directories, no project required), plus `--force` (destructive replace of same-name content) and `--json`. Installed copies carry a provenance header (`# source: core <cli-version>` / `# source: <standard-id> <version>`). **Supply-chain posture (ADR-037 D10):** skills committed to a repo are visible to every developer's agent in that repo — the intended team-distribution workflow, a trust boundary like `AGENTS.md`; Anvil guarantees the integrity and provenance of what it installs and **never executes skill content**. Unsupported agents (Continue, Aider, GitHub Copilot; Roo is covered by the `.agents/skills` master copy but not a selectable `--agent` value) are rejected with a notice. See [wiki/skills.md](wiki/skills.md) for the full reference.
