...
func downloadReleaseAssetFromURLClient(get func(string) (*http.Response, error), downloadURL, assetName, tmpPath string) (string, error) {
	...
	// Bound the body read by ACTIVITY, not by a total deadline: an
	// adapter binary can legitimately take longer than any fixed window
	// on a slow link, so a download that keeps delivering data runs as
	// long as it needs, while a stalled read fails within the idle
	// window (ANVIL_DOWNLOAD_IDLE_TIMEOUT, 30s default) with a clear
	// timeout error (TD-008 — no request can hang indefinitely).
...
func verifyChecksumFromURLClient(get func(string) (*http.Response, error), checksumsURL, binaryName, downloadedHash string) error {
	...
	// Bound the body read by ACTIVITY, not by a total deadline: the
	// checksum file rides the same download client as the binary it
	// verifies (no total per-request timeout), so the read is bounded
	// by the idle window (ANVIL_DOWNLOAD_IDLE_TIMEOUT, 30s default) —
	// a stalled fetch fails with a clear timeout error instead of
	// hanging (TD-008).
	resp.Body = newIdleTimeoutBody(resp.Body, downloadIdleTimeout())
