...
func runAdapterInspect(cmd *cobra.Command, args []string) error {
	...
	// The adapter must be discovered on the system and probe-validated
	// (TS-007-039) before executable resolution, so the error names the
	// adapter itself, not a missing binary. The known-framework list is
	// only a display fallback for the hint (AC-5).
