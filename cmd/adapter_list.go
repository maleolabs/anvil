...
// manager show up too. Each detected binary is probe-validated through
// its capabilities command (TS-007-039 §7): binaries that fail the probe
// — foreign or broken "anvil-adapter-*" executables — are excluded, and
// the remaining adapters' capabilities are invoked to display their real
// deployment model. When nothing valid is installed, the command says so
// and hints at "--available".
//
...
on PATH. Only executables that answer the capabilities command are
listed — binaries that fail the probe are treated as foreign and
excluded (TS-007-039). Deployment models come from each adapter's own
...
// on this system (CLI install directory first, then PATH) and pass the
// capabilities probe.
//
// Reference: TS-007-031, TS-007-039
func runAdapterListInstalled(cmd *cobra.Command, jsonOutput bool) error {
	// Probe-validate the detected binaries: a binary is a valid adapter
	// only when it answers the capabilities command, so foreign or broken
	// "anvil-adapter-*" executables never appear in the list (TS-007-039
	// AC-4) — the old "broken adapter shows unknown" behavior is replaced
	// by exclusion.
	...
	for _, name := range names {
		entries = append(entries, collectAdapterListEntry(cmd.Context(), name, adapters[name]))
	}
...
//
// Reference: TS-007-031, TS-007-039, 005-adapter-command-contract §10
func installedAdaptersFromSystem() map[string]string {
	...
	pathAdapters, err := scanPathAdapters()
	if err != nil {
		return found // a PATH scanning failure never blocks detection of the CLI dir
	}
	for name, executable := range pathAdapters {
...
// a broken adapter never fails the whole list. In default mode the
// candidates are probe-validated first (resolveAdapterSet, TS-007-039),
// so failures do not reach this function; the "unknown" fallback serves
// the --available installed-status marking.
