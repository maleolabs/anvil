...
func runAdapterListInstalled(cmd *cobra.Command, jsonOutput bool) error {
	...
	if len(installed) == 0 {
		...
		fmt.Fprintln(s.W, "No adapters installed.")
		fmt.Fprintln(s.W, "Run 'anvil adapter list --available' to see the adapters offered in the registry index.")
		return nil
	}
...
func runAdapterListAvailable(cmd *cobra.Command, jsonOutput bool) error {
	...
	if len(byAdapter) == 0 {
		...
		fmt.Fprintln(s.W, "No adapters available in the registry index.")
		fmt.Fprintln(s.W, "Set the index directory with --index <path> or the "+envStandardIndex+" environment variable.")
		return nil
	}
