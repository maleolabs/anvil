...
// binaries are the only detected adapters. The adapterKnownFrameworks
// seam is stubbed to control the fallback hint and the install/uninstall
// whitelist deterministically.
//
...
// stubAdapterReleaseFetch replaces the adapterReleaseFetch seam (the
// latest-release adapter list) and registers cleanup.
func stubAdapterReleaseFetch(t *testing.T, tag string, names []string, err error) {
	t.Helper()
	orig := adapterReleaseFetch
	adapterReleaseFetch = func() (string, []string, error) { return tag, names, err }
	t.Cleanup(func() { adapterReleaseFetch = orig })
}

// TestAdapterList_TableOutput verifies that "anvil adapter list" renders
// every adapter detected on the system with its deployment model.
//
// Reference: TS-007-031 AC-1, AC-2
func TestAdapterList_TableOutput(t *testing.T) {
	dir := t.TempDir()
	stubAdapterInstallDirAt(t, dir)
	t.Setenv("PATH", "") // deterministic: only the CLI install dir is scanned
...
// TestAdapterList_EmptyMessage verifies the empty state: when no adapter
// binaries exist on the system, the command says so and hints at the
// --available discovery flag (exit 0).
//
...
// TestAdapterList_InstalledOnPathOnly verifies that adapters installed
// outside the CLI directory are still detected — the scan covers PATH, so
// manually placed binaries show up too (the check is not static).
//
// Reference: TS-007-031, 005-adapter-command-contract §10
func TestAdapterList_InstalledOnPathOnly(t *testing.T) {
	pathDir := t.TempDir()
	stubAdapterInstallDirAt(t, t.TempDir()) // CLI dir has nothing installed
	t.Setenv("PATH", pathDir)
	writeFakeAdapter(t, pathDir, "anvil-adapter-laravel",
		`{"capabilities":{"deployment_model":"server"}}`,
		`{"extension":{"framework":"laravel","keys":[]}}`)
	...
	if !strings.Contains(stdout, "laravel") || !strings.Contains(stdout, "server") {
		t.Errorf("stdout should list the PATH-installed adapter with its model, got:\n%s", stdout)
	}
}

// TestAdapterList_ProbeFailureExcludesAdapter verifies that an installed
// binary whose capabilities request fails — a broken or foreign
// "anvil-adapter-*" executable — is excluded from the list (probe-based
// filtering, TS-007-039 AC-4) instead of showing an "unknown" state.
//
// Reference: TS-007-031 AC-1, TS-007-039 §7
func TestAdapterList_ProbeFailureExcludesAdapter(t *testing.T) {
	dir := t.TempDir()
	stubAdapterInstallDirAt(t, dir)
	t.Setenv("PATH", "")
	...
	if strings.Contains(stdout, "laravel") {
		t.Errorf("stdout should exclude the adapter whose probe failed, got:\n%s", stdout)
	}
	if strings.Contains(stdout, "unknown") {
		t.Errorf("stdout should not show an 'unknown' state (probe-failing adapters are excluded), got:\n%s", stdout)
	}
...
// ── Adapter List --available (TS-007-031, TS-007-034) ────────────────

// TestAdapterListAvailable_ListsReleaseAdapters verifies that --available
// lists the adapters published in the latest release with their install
// status: installed adapters keep their real deployment model, others
// show "-" and "available".
//
// Reference: TS-007-031, TS-007-034
func TestAdapterListAvailable_ListsReleaseAdapters(t *testing.T) {
	dir := t.TempDir()
	stubAdapterInstallDirAt(t, dir)
	t.Setenv("PATH", "")
	writeFakeAdapter(t, dir, "anvil-adapter-laravel",
		`{"capabilities":{"deployment_model":"server"}}`,
		`{"extension":{"framework":"laravel","keys":[]}}`)
	stubAdapterReleaseFetch(t, "v0.7.0", []string{"flutter", "laravel"}, nil)

	_, stdout, stderr, err := executeCommand("adapter", "list", "--available")
	if err != nil {
	...
	for _, want := range []string{"Name", "Deployment Model", "Version", "Status",
		"laravel", "server", "installed", "flutter", "-", "available", "v0.7.0"} {
		if !strings.Contains(stdout, want) {
			t.Errorf("stdout should contain %q, got:\n%s", want, stdout)
		}
	}
...
// TestAdapterListAvailable_FetchError verifies that a failed release
// fetch produces a clear error naming the release.
//
// Reference: TS-007-031
func TestAdapterListAvailable_FetchError(t *testing.T) {
	stubAdapterReleaseFetch(t, "", nil, errors.New("GitHub API returned HTTP 500"))

	_, _, stderr, err := executeCommand("adapter", "list", "--available")
	if err == nil {
		t.Fatal("expected fetch error, got nil")
	}
	if !strings.Contains(stderr, "could not fetch release adapters") {
		t.Errorf("stderr should report the fetch failure, got: %s", stderr)
	}
}

// TestAdapterListAvailable_Empty verifies the empty release state: a
// release without adapter assets yields an informative message, exit 0.
//
// Reference: TS-007-031
func TestAdapterListAvailable_Empty(t *testing.T) {
	stubAdapterReleaseFetch(t, "v0.7.0", nil, nil)

	_, stdout, stderr, err := executeCommand("adapter", "list", "--available")
	if err != nil {
		t.Fatalf("adapter list --available returned unexpected error: %v (stderr: %s)", err, stderr)
	}
	if !strings.Contains(stdout, "No adapters available in the latest release.") {
		t.Errorf("stdout should contain the empty message, got:\n%s", stdout)
	}
}

// TestAdapterListAvailable_JSON verifies the --available --json shape:
// entries carry name, deployment model, release version, and status.
//
// Reference: TS-007-031 AC-3
func TestAdapterListAvailable_JSON(t *testing.T) {
	stubAdapterReleaseFetch(t, "v0.7.0", []string{"laravel"}, nil)

	// Deterministic environment: no adapter binaries installed on this
	// machine (PATH cleared, CLI install dir stubbed to an empty dir), so
	// the release entry is reported as "available" (TS-007-039 discovery).
	t.Setenv("PATH", "")
	emptyDir := t.TempDir()
	stubAdapterInstallDirAt(t, emptyDir)

	_, stdout, stderr, err := executeCommand("adapter", "list", "--available", "--json")
	if err != nil {
	...
	if entry.Version != "v0.7.0" {
		t.Errorf("entry version = %q, want %q", entry.Version, "v0.7.0")
	}
...
// TestAdapterInspect_UnknownAdapter verifies that an adapter name that is
// not discovered on the system produces a clear error naming the known
// adapters (the KnownFrameworks display fallback, TS-007-039 AC-5).
//
// Reference: TS-007-032 AC-4, TS-007-039
...
// TestAdapterInspect_ExecutableNotFound verifies that a discovered
// adapter whose executable cannot be resolved produces a clear error
// naming the expected binary.
//
