...
//
// This is a parent command group that serves as a namespace for artifact
...
// TS-019-04-02 ("artifact status", "artifact verify-immutability") —
// produce a clear "unknown command" error instead of silently falling
// back to group help with exit 0 (BUG-012 Validation step 4).
...
func runPackage(cmd *cobra.Command, args []string) error {
	...
	// subdirectory still packages the whole project (BUG-005).
	...
	// command strings come from the framework standard executable through
	// the manifest command (005-adapter-command-contract §10.10) — the
	// Core never imports framework packages (ADR-009 §8.1; the framework
	// packages left the Core module in TS-016-01-01 and TS-016-02-01,
	// ADR-025 §6.2). The adapter is OPTIONAL: a project without a
	// framework, without the adapter binary installed, or with a failing
	// manifest invocation keeps the pre-existing behavior and packages
	// without
	...
	if framework := activeProjectFramework(); framework != "" {
		// The guarded resolver validates the identifier BEFORE any
		// lookup (team review F1): a malicious project.framework with
		// path separators can never steer exec.LookPath at a
		// working-directory-relative executable — it degrades to the
		// missing-adapter warning instead. Resolution order: CLI
		// install dir first, then PATH (F2).
		executable, err := resolveAdapterExecutable(framework)
		if err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), FmtWarning("adapter executable %q for framework %q not resolvable (%v); packaging without manifest activation/rollback commands"), adapterBinaryName(framework), framework, err)
		} else {
	...
	w2 := s2.W
	fmt.Fprintf(w2, "\nArtifact created:\n")
	fmt.Fprintf(w2, "  Primary:   %s\n", result.ArtifactPath)
	if result.SecondaryPath != "" {
		fmt.Fprintf(w2, "  Secondary: %s\n", result.SecondaryPath)
	}
	fmt.Fprintf(w2, "  Files:     %d\n", result.FileCount)
	if manifestID != "" {
		shortID := manifestID
		if len(shortID) > 16 {
			shortID = shortID[:16] + "..."
