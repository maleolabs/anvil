...
func runArtifactStatus(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return ReportPlainError(cmd, err, "no Anvil project found")
	}

	// Ensure the state directory exists.
	if _, statErr := os.Stat(stateDir); statErr != nil {
		return ReportPlainErrorf(cmd, statErr, "state directory not found: %s", stateDir)
	}
	...
	if _, statErr := os.Stat(regPath); statErr == nil {
		if err := store.Load(); err != nil {
			return ReportPlainErrorf(cmd, err, "could not load registration index: %v", err)
		}
	}
	...
	if _, statErr := os.Stat(lifecyclePath); statErr == nil {
		if err := lsm.Load(lifecyclePath); err != nil {
			return ReportPlainErrorf(cmd, err, "could not load lifecycle state: %v", err)
		}
