...
func runVerify(cmd *cobra.Command, args []string) error {
	...
	if result.Passed {
		fmt.Fprintln(styleFor(cmd).W, "Artifact verification: PASSED")
		for _, check := range result.Checks {
			output.PrintStatus(styleFor(cmd).W, output.StatusPass, check.Name+": "+check.Details)
		}
	...
	fmt.Fprintln(styleFor(cmd).W, "Artifact verification: FAILED")
	for _, check := range result.Checks {
		if !check.Passed {
			...
			output.PrintStatus(styleFor(cmd).W, output.StatusFail, check.Name+": "+details)
		}
	}
...
func runFrameworkVerification(cmd *cobra.Command, artifactPath string) error {
	...
	// The guarded resolver validates the identifier BEFORE any lookup
	// (team review F1): a malicious project.framework with path
	// separators can never steer exec.LookPath at a
	// working-directory-relative executable — it degrades to the
	// missing-adapter warning instead. Resolution order: CLI install
	// dir first, then PATH (F2).
	executable, err := resolveAdapterExecutable(framework)
	if err != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), FmtWarning("adapter executable %q for framework %q not resolvable (%v); skipping framework verification checks"), adapterBinaryName(framework), framework, err)
		return nil
	}
	...
	fmt.Fprintln(styleFor(cmd).W, "Framework verification:")
	failed := false
	for _, check := range checks {
		...
		if outcome.Passed {
			output.PrintStatus(styleFor(cmd).W, output.StatusPass, name+": "+details)
			continue
		}
		output.PrintStatus(styleFor(cmd).W, output.StatusFail, name+": "+details)
		failed = true
	}
