...
func TestVerifyCmd_PassOutput(t *testing.T) {
	...
	// Each line should start with ✓ for passing checks.
	lines := strings.Split(strings.TrimSpace(stdout), "\n")
	for _, line := range lines[1:] { // Skip the header line
		if !strings.HasPrefix(line, "✓") {
			t.Errorf("expected check line to start with ✓, got: %s", line)
		}
	}
}
...
func TestVerifyCmd_FailOutput(t *testing.T) {
	...
	// Should have failed check with ✗.
	if !strings.Contains(stdout, "✗") {
		t.Errorf("expected ✗ for failed check, got:\n%s", stdout)
	}
}
