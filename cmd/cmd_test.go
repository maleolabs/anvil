...
//
// AC: Command groups exist for: project, artifact, pipeline,
...
func TestCommandHierarchy_GroupsAreParents(t *testing.T) {
	...
	skipRunCheck := map[string]bool{
		...
		"project":    true, // NoArgsWithSuggestions rejects unknown subcommands (BUG-012, TS-019-04-02)
		"artifact":   true, // NoArgsWithSuggestions rejects unknown subcommands (BUG-012, TS-019-04-02)
	}
...
func TestCommandConventions_NoRunWithoutRunE(t *testing.T) {
	...
	skip := map[string]bool{
		...
		"project":  true, // projectCmd displays group help when invoked directly (BUG-012, TS-019-04-02)
		"artifact": true, // artifactCmd displays group help when invoked directly (BUG-012, TS-019-04-02)
	}
...
func TestCommandConventions_ParentGroupsNoRunE(t *testing.T) {
	...
	skip := map[string]bool{
		...
		"project":    true, // projectCmd uses NoArgsWithSuggestions to reject ghost subcommands (BUG-012, TS-019-04-02)
		"artifact":   true, // artifactCmd uses NoArgsWithSuggestions to reject ghost subcommands (BUG-012, TS-019-04-02)
	}
...
// "anvil system --help" documents only the commands that exist (inspect)
// and no longer advertises the ghost "anvil system version" / "anvil
// system info" examples (BUG-012) or the demoted platform-ops surfaces
// "anvil system health" / "anvil system diagnose" (ADR-036 §3,
// TS-015-05-02).
//
// AC: anvil system --help examples match only the registered subcommands
// (BUG-012 Validation step 3; ADR-036 demotion).
func TestSystemHelp_DocumentsOnlyRegisteredCommands(t *testing.T) {
	...
	for _, want := range []string{"inspect"} {
	...
	// The help must not advertise the demoted platform-ops surfaces
	// (ADR-036 §3, TS-015-05-02).
	for _, demoted := range []string{"health", "diagnose"} {
		if strings.Contains(stdout, demoted) {
			t.Errorf("system --help must not document the demoted %q surface (ADR-036), got:\n%s", demoted, stdout)
		}
	}
