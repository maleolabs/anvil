...
// TestConfigValidateCommand_MalformedConfig verifies that a malformed
// configuration source exits with the config category (2, TS-019-03-02
// D-04): malformed configuration is "invalid" per the global contract
// table, so it is a configuration error — not a general resolution
// failure — and carries the categorized exit code.
func TestConfigValidateCommand_MalformedConfig(t *testing.T) {
	isolateConfigEnvironment(t)
	dir := t.TempDir()
	configPath := filepath.Join(dir, "anvil.yaml")
	...
	if !contains(stderr, "invalid") {
		t.Errorf("stderr should report the invalid configuration, got: %s", stderr)
	}
}

// TestConfigValidateCommand_UnreadableConfig verifies that a
// configuration source that cannot be read at all (I/O failure) stays a
// general error (exit 1, TS-019-03-02 D-04) — only malformed content is
// classified as invalid configuration (exit 2). An environment config
// path that is a directory cannot be read as a file — a deterministic
// I/O failure independent of process permissions.
func TestConfigValidateCommand_UnreadableConfig(t *testing.T) {
	isolateConfigEnvironment(t)
	t.Setenv("ANVIL_ENV", "broken")
	dir := t.TempDir()
	// A minimal valid project marker so the project root is discovered.
	if err := os.WriteFile(filepath.Join(dir, "anvil.yaml"), []byte("project:\n  name: unreadable-test\n"), 0644); err != nil {
		t.Fatalf("failed to write config file: %v", err)
	}
	// The environment config path exists but is a directory: reading it
	// is an I/O failure, not a parse failure.
	envDir := filepath.Join(dir, "config", "environments")
	if err := os.MkdirAll(envDir, 0755); err != nil {
		t.Fatalf("failed to create environment config dir: %v", err)
	}
	if err := os.Mkdir(filepath.Join(envDir, "broken.yaml"), 0755); err != nil {
		t.Fatalf("failed to create environment config path: %v", err)
	}
...
// TestConfigValidateCommand_MalformedConfigJSON verifies that a
// malformed configuration source exits non-zero (config category, 2) and
// emits the standard JSON error envelope when --json is used — a
// malformed source is not a validation result, so no categorized
// validation data is produced.
func TestConfigValidateCommand_MalformedConfigJSON(t *testing.T) {
	isolateConfigEnvironment(t)
	dir := t.TempDir()
	configPath := filepath.Join(dir, "anvil.yaml")
	if err := os.WriteFile(configPath, []byte("project: [unclosed"), 0644); err != nil {
