...
// (TS-P8-08 AC4).
//
// Server commands validate inputs at different layers:
//   - "server release install" validates artifact existence first
//   - "server release activate/rollback" validate project registration
//   - "server release cleanup" validates project registration
//
...
func TestCrossContext_ErrorMessages(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantGuidance:   true,
			wantErrKey:     "artifact not found",
			providesAction: true, // "Check that the artifact path is correct"
		},
		{
			...
			providesAction: false, // improvement opportunity: add "register the project first" guidance
		},
		{
		...
		},
		{
			...
			wantGuidance:   true,
			wantErrKey:     "not registered",
			providesAction: true, // "Register the project first using 'anvil server project register'"
		},
		{
			name:           "server project get without init",
			...
			wantGuidance:   true,
			wantErrKey:     "not found",
			providesAction: false, // improvement opportunity: add "run server init" guidance
		},
	}
