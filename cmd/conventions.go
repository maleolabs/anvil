...
// Output (ADR-010 §3.4):
//
//	Human-readable: write to styleFor(cmd).W
//	Errors/warnings: write to cmd.ErrOrStderr()
//	Machine-readable: use --json flag, write JSON to styleFor(cmd).W
//
// Error Reporting:
//
...
//
//	Exception: groups whose unknown subcommands would otherwise silently
//	fall back to group help with exit 0 use the serverReleaseCmd pattern
//	(serverReleaseCmd, systemCmd): a help-displaying Run combined with
//	Args: NoArgsWithSuggestions() so ghost command surfaces fail with a
//	clear "unknown command" error (BUG-012).
//
...
func PrintKeyValue(cmd *cobra.Command, key string, value interface{}) {
	fmt.Fprintf(styleFor(cmd).W, "  %s: %v\n", key, value)
}
...
func PrintSuccess(cmd *cobra.Command, message string) {
	output.PrintSuccess(styleFor(cmd).W, message)
}
...
func PrintSuccessf(cmd *cobra.Command, format string, args ...interface{}) {
	output.PrintSuccessf(styleFor(cmd).W, format, args...)
}
...
func WriteJSON(cmd *cobra.Command, v interface{}) error {
	return output.WriteJSON(styleFor(cmd).Raw(), v)
}
...
func WriteJSONError(cmd *cobra.Command, errMsg string) error {
	return output.WriteJSONError(styleFor(cmd).W, errMsg)
}
...
func PrintSection(cmd *cobra.Command, title string) {
	fmt.Fprintln(styleFor(cmd).W, title)
...
func PrintStatus(cmd *cobra.Command, status output.Status, message string) {
	output.PrintStatus(styleFor(cmd).W, status, message)
}
...
func ReportError(cmd *cobra.Command, e *output.AppError) error {
	...
	if jsonFlag {
		// The error envelope is written to stdout, and the original
		// error is still returned so the process exits with the
		// category code — a failure must never exit 0 (TS-019-03-02).
		_ = output.WriteJSONError(styleFor(cmd).W, e.Error())
		return markReported(e)
	}
	output.WriteAppError(cmd.ErrOrStderr(), e)
	return markReported(e)
}

// reportedError marks an error whose user-facing rendering already
// happened (ReportError/ReportPlainError/WriteAppError): Execute's central
// printer must not echo it a second time. It unwraps to the original error
// so errors.As/Is chains (ExitCoder, AppError assertions) keep working.
type reportedError struct{ err error }

func (r *reportedError) Error() string { return r.err.Error() }
func (r *reportedError) Unwrap() error { return r.err }

func markReported(err error) error { return &reportedError{err: err} }
...
func ReportPlainError(cmd *cobra.Command, err error, message string) error {
	...
	if jsonFlag {
		// The error envelope is written to stdout, and the original
		// error is still returned so the process exits non-zero — a
		// failure must never exit 0 (TS-019-03-02).
		_ = output.WriteJSONError(styleFor(cmd).W, message)
