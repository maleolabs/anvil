...
func runDeploy(cmd *cobra.Command, _ []string) error {
	...
	if !vr.Passed {
		...
		if humanReporter != nil {
			...
			fmt.Fprintf(styleFor(cmd).W, "Deploy dry-run --target %s\n", target)
			for _, c := range vr.Checks {
				st := output.StatusPass
				if !c.Passed {
					st = output.StatusFail
	...
	if !dryRun && shouldValidateServerTarget {
		...
		if humanReporter != nil {
			...
			if info, err := os.Stat(result.ArtifactPath); err == nil {
				...
				output.EmitPushProgress(styleFor(cmd).W, base, info.Size())
			}
...
func (d *deployTarget) CompatibilityInput() deployment.CompatibilityInput {
