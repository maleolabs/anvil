...
--json output so automation can correlate the operation, but it does
NOT select a target. The operation runs on the local server runtime.
...
Examples:
  ...
  anvil deployment activate my-target my-project abc123def456 --server-root /tmp/anvil
  anvil deployment activate my-target my-project abc123def456 --json`,
