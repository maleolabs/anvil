...
func TestDeploymentActivateCommand_RegistersUnderDeployment(t *testing.T) {
	...
	if sub.Use != "activate <target-id> <project-id> <release-id>" {
		t.Errorf("command Use = %q, want %q", sub.Use, "activate <target-id> <project-id> <release-id>")
	}
...
//
//	anvil deployment activate <target-id> <project-id> <release-id> --server-root <dir>
//
...
func TestDeploymentActivateCommand_UninitializedRuntime(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "activate", "my-target", "my-project", "rel-001", "--server-root", dir)
	if err == nil {
...
func TestDeploymentActivateCommand_ActivatesRelease(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "activate", "my-target", projectID, releaseID, "--server-root", serverRoot)
	if err != nil {
...
//
//	anvil deployment activate <target-id> <project-id> <release-id> --server-root <dir> --json
//
...
func TestDeploymentActivateCommand_JSONOutput(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "activate", "my-target", projectID, releaseID, "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("activate command returned unexpected error: %v\nstderr: %s", err, stderr)
	}
...
func TestDeploymentActivateCommand_ProjectNotRegistered(t *testing.T) {
	...
	_, _, stderr, err := executeCommand("deployment", "activate", "my-target", "nonexistent-project", "rel-001", "--server-root", serverRoot)
	if err == nil {
...
//	anvil deployment activate
//
// requires exactly three positional arguments.
func TestDeploymentActivateCommand_ExactArgs(t *testing.T) {
	...
	if !contains(stderr, "requires 3 argument") {
		t.Errorf("stderr should require target-id, project-id and release-id, got: %s", stderr)
	}

	// Test with two args.
	_, _, stderr2, err2 := executeCommand("deployment", "activate", "target-1", "project-1")
	if err2 == nil {
		t.Fatal("expected error for missing release-id, got nil")
	}
	if !contains(stderr2, "requires 3 argument") {
		t.Errorf("stderr should require exactly 3 args, got: %s", stderr2)
	}

	// Test with four args.
	_, _, stderr3, err3 := executeCommand("deployment", "activate", "target-1", "project-1", "rel-1", "extra-arg")
	if err3 == nil {
		t.Fatal("expected error for extra args, got nil")
	}
	if !contains(stderr3, "requires 3 argument") {
		t.Errorf("stderr should require exactly 3 args, got: %s", stderr3)
	}
}
...
func TestDeploymentActivateCommand_NonDefaultServerRoot(t *testing.T) {
	...
	_, _, stderr, _ := executeCommand("deployment", "activate", "my-target", "test-project", "rel-001", "--server-root", customRoot)
