...
func runDeploymentInfo(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(styleFor(cmd).W, "Deployment Target\n")
	fmt.Fprintf(styleFor(cmd).W, "  Target Name:   %s\n", deliveryInfo.TargetName)
	fmt.Fprintf(styleFor(cmd).W, "  Runtime ID:    %s\n", deliveryInfo.RuntimeID)
	fmt.Fprintf(styleFor(cmd).W, "  Config:        %s\n", deliveryInfo.ConfigPath)
	fmt.Fprintf(styleFor(cmd).W, "  Delivery:      %s\n", deliveryInfo.DeliveryStatus)
	fmt.Fprintln(styleFor(cmd).W, "")
	fmt.Fprintln(styleFor(cmd).W, "Deployment information is distinct from Runtime status.")
	fmt.Fprintln(styleFor(cmd).W, "Use 'anvil server status' to inspect Runtime state.")
