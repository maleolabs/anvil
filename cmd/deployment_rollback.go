...
func runDeploymentRollback(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(styleFor(cmd).W, "Rollback completed.\n")
	fmt.Fprintf(styleFor(cmd).W, "  Project ID:               %s\n", projectID)
	fmt.Fprintf(styleFor(cmd).W, "  Rolled Back Release ID:   %s\n", result.RolledBackRelease.ID)
	fmt.Fprintf(styleFor(cmd).W, "  Rolled Back Stage:        %s\n", result.RolledBackRelease.Stage)
	fmt.Fprintf(styleFor(cmd).W, "  Restored Release ID:      %s\n", result.RestoredRelease.ID)
	fmt.Fprintf(styleFor(cmd).W, "  Restored Stage:           %s\n", result.RestoredRelease.Stage)
