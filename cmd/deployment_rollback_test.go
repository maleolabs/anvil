...
func TestDeploymentRollbackCommand_RegistersUnderDeployment(t *testing.T) {
	...
	if sub.Use != "rollback <project-id>" {
		t.Errorf("command Use = %q, want %q", sub.Use, "rollback <project-id>")
	}
...
//
//	anvil deployment rollback <project-id> --server-root <dir>
//
...
func TestDeploymentRollbackCommand_UninitializedRuntime(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "rollback", "my-project", "--server-root", dir)
	if err == nil {
...
func TestDeploymentRollbackCommand_RollbackSuccess(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "rollback", projectID, "--server-root", serverRoot)
	if err != nil {
...
//
//	anvil deployment rollback <project-id> --server-root <dir> --json
//
...
func TestDeploymentRollbackCommand_JSONOutput(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("deployment", "rollback", projectID, "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("rollback command returned unexpected error: %v\nstderr: %s", err, stderr)
	}
...
func TestDeploymentRollbackCommand_ProjectNotRegistered(t *testing.T) {
	...
	_, _, stderr, err := executeCommand("deployment", "rollback", "nonexistent-project", "--server-root", serverRoot)
	if err == nil {
...
func TestDeploymentRollbackCommand_NoActiveRelease(t *testing.T) {
	...
	_, _, stderr, err := executeCommand("deployment", "rollback", projectID, "--server-root", serverRoot)
	if err == nil {
...
func TestDeploymentRollbackCommand_ExactArgs(t *testing.T) {
	...
	if !contains(stderr, "requires 1 argument") {
		t.Errorf("stderr should require project-id, got: %s", stderr)
	}

	// Test with two args (the pre-removal form "<target-id> <project-id>"):
	// the phantom target-id argument must be rejected per the announced
	// deprecation schedule (TS-019-04-03, ADR-032 D10).
	_, _, stderr2, err2 := executeCommand("deployment", "rollback", "target-1", "project-1")
	if err2 == nil {
	...
	if !contains(stderr3, "requires 1 argument") {
		t.Errorf("stderr should require exactly 1 arg, got: %s", stderr3)
	}
}
...
func TestDeploymentRollbackCommand_NonDefaultServerRoot(t *testing.T) {
	...
	_, _, stderr, _ := executeCommand("deployment", "rollback", projectID, "--server-root", customRoot)
