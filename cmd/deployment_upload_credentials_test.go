...
// prepareUploadCredentialTest initializes the server root and writes a
// valid artifact file for credential tests, returning the server root
// dir and the artifact path to use with executeCommand.
...
func TestDeploymentUploadCommand_MissingCredentialHost(t *testing.T) {
	dir, artifactPath := prepareUploadCredentialTest(t)
