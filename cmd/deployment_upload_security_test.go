...
func TestDeploymentUploadCommand_Security_JSONOutput_DoesNotLeakCredentials(t *testing.T) {
	...
	artifactPath := filepath.Join(t.TempDir(), "security-json-artifact.tar.gz")
	if err := os.WriteFile(artifactPath, []byte("artifact content for security JSON test"), 0644); err != nil {
		t.Fatalf("failed to create artifact: %v", err)
	}

	setUploadEnv(t, securitySentinelHost, server.Port(), securitySentinelUser, keyPath)
	t.Cleanup(func() {
		_ = os.Remove(path.Join("/tmp/anvil-uploads", "security-json-artifact.tar.gz"))
	})
...
func TestDeploymentUploadCommand_Security_HumanOutput_DoesNotLeakCredentials(t *testing.T) {
	...
	artifactPath := filepath.Join(t.TempDir(), "security-artifact.tar.gz")
	if err := os.WriteFile(artifactPath, []byte("artifact content for security test"), 0644); err != nil {
		t.Fatalf("failed to create artifact: %v", err)
	}

	setUploadEnv(t, securitySentinelHost, server.Port(), securitySentinelUser, keyPath)
	t.Cleanup(func() {
		_ = os.Remove(path.Join("/tmp/anvil-uploads", "security-artifact.tar.gz"))
	})
