...
func TestTruthfulExitCodes_JSONErrorEnvelope_ServerReleaseHistory(t *testing.T) {
	...
	if !contains(stderr, "not initialized") {
		t.Errorf("stderr should carry the plain error presentation, got: %s", stderr)
	}
}

// TestTruthfulExitCodes_JSONErrorEnvelope_StandardListMissingIndex
// verifies that "standard list --json" with no registry index exits 3
// (not found) with the error envelope on stdout.
func TestTruthfulExitCodes_JSONErrorEnvelope_StandardListMissingIndex(t *testing.T) {
	...
	if !strings.Contains(envelope.Error, "index") {
		t.Errorf("envelope error = %q, want the registry-index failure named", envelope.Error)
	}
	if contains(stdout, "Error:") {
		t.Errorf("plain error presentation must not be echoed on stdout, got: %s", stdout)
	}
	if !contains(stderr, "index") {
		t.Errorf("stderr should carry the plain error presentation, got: %s", stderr)
	}
}
