...
Command-specific notes:
  ...
  - "anvil skill install --all" exits 0 when every skill succeeds;
    1 when one or more skills fail (per-skill errors in output;
    JSON envelope carries per-skill results with exit_code per row);
    3 when no skills are available; 4 on a precondition failure
  - "anvil skill install" (no name) exits 1 when the interactive
    form is unavailable — stdin is not a terminal or --json was
    passed (the automation surface never opens the form); 1 when the
    user cancels the form (Ctrl-C / Esc, nothing installed); 3 when
    no skills are available; 4 on a repo-scope precondition failure
    (no Anvil project/git root)
...
var helpExitCodesCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		_, err := fmt.Fprintln(styleFor(cmd).W, exitCodesDetail)
		return err
	},
}
