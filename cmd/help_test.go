...
// TestHelpExitCodesCommand_SystemInspectRows verifies that the
// exit-codes topic documents the "anvil system inspect" command-specific
// rows added to docs/operations/exit-codes.md by TS-020-01-02, with the
// truthful caveats: exit 0 when all checks pass, exit 1 on failed
// check(s) in human mode (with --json the failed checks are reported
// per check in the envelope and the process exits 0), exit 3 when the
// Release is not found, and deps never gating (informational).
//
// AC: CLI topic parity with the normative reference (TS-019-03-03 DoD).
func TestHelpExitCodesCommand_SystemInspectRows(t *testing.T) {
	_, stdout, stderr, err := executeCommand("help", "exit-codes")
	if err != nil {
		t.Fatalf("executeCommand('help', 'exit-codes') returned unexpected error: %v", err)
	}
	...
	expected := []string{
		...
		"exits 0 when all checks pass — informational output",
		...
		"anvil system inspect release",
		"exits 3 when the Release is not",
		"found — no Release with the given identity exists (runtime)",
		"the not-found lookup gates with 3 per D-03",
...
// TestHelpExitCodesCommand_SkillRows verifies that the exit-codes
// topic documents the "anvil skill" command-specific rows added to
// docs/operations/exit-codes.md by TS-021-05 (ADR-037 D10): list
// surfaces stale/unreadable records without gating (0), install and
// update map not-found to 3 and conflicts/version conflicts to 2,
// preconditions to 4, and uninstall is graceful (0) when the skill is
// not installed, with the filter-mismatch row at 3.
//
// AC: CLI topic parity with the normative reference (TS-021-05 DoD).
func TestHelpExitCodesCommand_SkillRows(t *testing.T) {
	_, stdout, stderr, err := executeCommand("help", "exit-codes")
	if err != nil {
		t.Fatalf("executeCommand('help', 'exit-codes') returned unexpected error: %v", err)
	}
	...
	expected := []string{
		...
		"install never changes versions — update is the",
		"4 when a precondition is missing (no selectable",
		`"anvil skill install --all" exits 0 when every skill succeeds`,
		"1 when one or more skills fail",
...
// TestHelpExitCodesCommand_AdapterSurfaceExcludedFromEnforcement
// verifies that the exit-codes topic documents the D-09 clause: the
// deprecated v1.x adapter surface is excluded from exit-code
// enforcement (ADR-032), matching docs/operations/exit-codes.md.
//
// AC: D-09 clause present in the CLI topic (TS-019-03-03 DoD).
func TestHelpExitCodesCommand_AdapterSurfaceExcludedFromEnforcement(t *testing.T) {
	_, stdout, stderr, err := executeCommand("help", "exit-codes")
	if err != nil {
		t.Fatalf("executeCommand('help', 'exit-codes') returned unexpected error: %v", err)
	}
