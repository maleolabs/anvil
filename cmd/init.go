...
func init() {
	...
	initCmd.Flags().String("framework", "", "Framework for template generation (e.g. laravel)")
}
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	result, err := engine.Initialize(name, path, engine.WithFramework(framework))
	if err != nil {
	...
	case engine.ResultCreated:
		if framework != "" {
			PrintSuccessf(cmd, "Project '%s' created with '%s' framework. Ready for use.", name, framework)
		} else {
