...
func buildGUIBundle(payloadPath, outputPath, target, installerName, installerIcon string, flatCfg map[string]interface{}, guiFormat string) error {
	...
	frontendDir := filepath.Join(buildRoot, "installer-gui")
	frontend := exec.Command("npm", "run", "build")
	frontend.Dir = frontendDir
	var frontendLog bytes.Buffer
	frontend.Stdout = io.MultiWriter(os.Stdout, &frontendLog)
	frontend.Stderr = io.MultiWriter(os.Stderr, &frontendLog)
	if err := frontend.Run(); err != nil {
		return fmt.Errorf("build GUI frontend failed: %w; %s", err, summarizeBuildLog(frontendLog.String()))
	}
	if err := disableStagedGUIBuildHook(filepath.Join(tauriDir, "tauri.conf.json")); err != nil {
		return fmt.Errorf("disable staged GUI build hook: %w", err)
	}
...
func disableStagedGUIBuildHook(configPath string) error {
	...
	if build, ok := conf["build"].(map[string]interface{}); ok {
