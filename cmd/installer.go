...
func ensureGUIScaffold(projectRoot, installerName, installerIcon string) error {
	...
	// Tauri requires an icon even when user does not customize one.
	iconPath := filepath.Join(tauriDir, "icons/icon.png")
	if _, err := os.Stat(iconPath); os.IsNotExist(err) {
		icon, err := base64.StdEncoding.DecodeString("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4n8bwHwAGMgJlMwnCZQAAAABJRU5ErkJggg==")
		if err != nil {
			return err
		}
  ...
  "bundle": {
    ...
    "resources": ["resources/installer-artifact.tar.gz", "resources/installer-artifact.sha256", "resources/forms.json", "resources/setup.json"]
  },
  "app": {
...
`, title, ident, title)
	...
	if installerIcon != "" {
		...
		if data, err := os.ReadFile(srcIcon); err == nil {
			_ = os.WriteFile(filepath.Join(tauriDir, "icons/icon.png"), data, 0o644)
		}
	}
...
func normalizeGUIPackage(base string) error {
	...
	// Svelte 4 requires vite-plugin-svelte 3.
	dev["@sveltejs/vite-plugin-svelte"] = "^3"
	dev["svelte"] = "^4"
	dev["vite"] = "^5"
...
func normalizeGUIConfig(projectRoot string) error {
	...
	if build["beforeBuildCommand"] == nil {
...
func updateGUIScaffoldNameIcon(projectRoot, installerName, installerIcon string) error {
	...
	if installerIcon != "" {
		...
		if iconData, err := os.ReadFile(srcIcon); err == nil {
			_ = os.WriteFile(filepath.Join(projectRoot, "installer-gui/src-tauri/icons/icon.png"), iconData, 0o644)
