...
func runProjectRemove(cmd *cobra.Command, args []string) error {
	...
	if !force {
		...
		fmt.Fprintf(s.W, "Warning: You are about to remove project '%s'.\n", projectName)
		fmt.Fprintf(s.W, "This will delete the project directory and all its contents.\n")
		fmt.Fprintf(s.W, "Are you sure you want to remove project '%s'? This will delete the project directory and all its contents. [y/N] ", projectName)
		...
		if err != nil {
			...
			return ReportError(cmd, &output.AppError{
				Message:    "non-interactive mode requires --force to remove a project",
				Reason:     "Project removal is a destructive operation that requires explicit confirmation in non-interactive mode",
				Resolution: "Re-run with --force flag to confirm removal: anvil project remove --force",
				Err:        fmt.Errorf("non-interactive removal requires --force"),
			})
		}

		response = strings.TrimSpace(strings.ToLower(response))
		if response != "y" && response != "yes" {
			fmt.Fprintln(s.W, "Removal cancelled.")
			return nil
		}
	}
	...
	if err != nil {
		...
		return ReportPlainErrorf(cmd, err, "could not locate project root: %v", err)
	}

	if err := project.RemoveProject(root); err != nil {
		return ReportPlainErrorf(cmd, err, "could not remove project: %v", err)
	}

	PrintSuccessf(cmd, "Project '%s' has been removed.", projectName)
	return nil
}
