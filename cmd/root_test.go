...
func TestCommandHelp_HelpExitCodeZero(t *testing.T) {
	...
	tests := []helpTest{
		...
		{[]string{"runtime", "--help"}, "runtime --help"},
		{[]string{"runtime", "list", "--help"}, "runtime list --help"},
		{[]string{"runtime", "status", "--help"}, "runtime status --help"},
		{[]string{"runtime", "readiness", "--help"}, "runtime readiness --help"},
		{[]string{"runtime", "provision", "--help"}, "runtime provision --help"},
		{[]string{"runtime", "verify-shared", "--help"}, "runtime verify-shared --help"},
...
func TestMissingArgGuidance_ShowsUsageExample(t *testing.T) {
	_, _, stderr, err := executeCommand("artifact", "status")
	if err == nil {
		t.Fatal("expected error for missing artifact identity, got nil")
	}

	// The error should include the command path.
	if !contains(stderr, "artifact status") {
	...
	if !contains(stderr, "<identity>") {
		t.Errorf("stderr should include the argument name <identity>, got: %s", stderr)
	}
}
...
func TestMissingArgGuidance_ValidCommand_NoGuidance(t *testing.T) {
	// artifact status requires 1 arg. We pass a dummy value; the command
	// may fail later due to project context, but arg validation should pass.
	_, _, stderr, err := executeCommand("artifact", "status", "dummy-identity")
	if err == nil {
		t.Fatal("expected error (project context), got nil")
	}
