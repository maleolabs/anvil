...
// Known gap: the legacy multi-runtime registry (runtimes.json) has no
// server-side equivalent — see docs/migration-guide-v1.5.md.
//
// Reference: ST-P5-06
...
DEPRECATED: "anvil runtime list" is deprecated. Use "anvil server status"
instead (ADR-021 migration table, docs/migration-guide-v1.5.md). Known gap:
the legacy multi-runtime registry has no server-side equivalent and is
tracked as follow-up work. This command is retained for backward
compatibility and will be removed in a future release — behavior and exit
codes are unchanged.
...
This is a read-only command — it does not modify any state.`,
  ...
  anvil runtime list --runtimes-path /opt/anvil/runtimes.json`,
	Deprecated:   `use "anvil server status" instead; note: the legacy multi-runtime registry has no server-side equivalent (see docs/migration-guide-v1.5.md)`,
	SilenceUsage: true,
	RunE:         runList,
}
