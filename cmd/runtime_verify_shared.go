...
//
// DEPRECATED: use "anvil server doctor" instead (ADR-021 migration table).
//
// Reference: ST-P5-05
...
DEPRECATED: "anvil runtime verify-shared" is deprecated. Use "anvil server
doctor" instead (ADR-021 migration table, docs/migration-guide-v1.5.md). This
command is retained for backward compatibility and will be removed in a
future release — behavior and exit codes are unchanged.
...
It uses SharedResourceManager to perform the checks.`,
  ...
  anvil runtime verify-shared --install-root /opt/anvil`,
	Deprecated:   `use "anvil server doctor" instead (see docs/migration-guide-v1.5.md)`,
	SilenceUsage: true,
...
func runVerifyShared(cmd *cobra.Command, args []string) error {
	...
	if allOK {
		PrintSuccess(cmd, "Shared resources are intact.")
		return nil
	}
