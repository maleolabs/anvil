...
//
// It resolves the config root path, loads the configuration, and displays
// the requested value.
func runServerConfigGet(cmd *cobra.Command, args []string) error {
	...
	if !store.Exists() {
		return fmt.Errorf("server config not found at %s: run 'anvil server init' first", store.ConfigPath())
	}
