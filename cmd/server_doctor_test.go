...
// code 0 — the assessment is demoted, non-governing diagnostics
...
//
// Reference: ST-P9-01 (ST-009-001 AC2), ADR-036
func TestServerDoctorCommand_Degraded(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("server", "doctor", "--server-root", dir)
	if err != nil {
		t.Fatalf("demoted doctor command must exit 0 even when degraded, got error: %v", err)
	}
	...
	if stderr != "" {
		t.Errorf("demoted doctor command must not write a failure error, got stderr: %s", stderr)
	}
}

// TestServerDoctorCommand_Unhealthy verifies that when all checks fail,
// the doctor reports "Unhealthy" and exits with code 0 — the assessment is
// demoted, non-governing diagnostics (ADR-036 §3, TS-015-05-02).
//
...
//
// Reference: ST-P9-01 (ST-009-001 AC3), ADR-036
func TestServerDoctorCommand_Unhealthy(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("demoted doctor command must exit 0 even when unhealthy, got error: %v", err)
	}
...
// status and the command exits 0 (demoted, non-governing diagnostics —
// ADR-036 §3, TS-015-05-02).
//
// Reference: ST-P9-01, ADR-036
...
// command exits 0 (demoted, non-governing diagnostics — ADR-036 §3,
// TS-015-05-02).
//
...
func TestServerDoctorCommand_JsonNotHealthy(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("demoted doctor command must exit 0 even when not healthy, got error: %v", err)
	}
