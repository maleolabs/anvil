...
func runServerInit(cmd *cobra.Command, args []string) error {
	...
	if result.AlreadyInitialized {
		fmt.Fprintf(cmd.OutOrStdout(), "Server Runtime already initialized.\n")
		fmt.Fprintf(cmd.OutOrStdout(), "  Config: %s\n", result.ConfigPath)
	} else {
		fmt.Fprintf(cmd.OutOrStdout(), "Server Runtime initialized.\n")
		fmt.Fprintf(cmd.OutOrStdout(), "  Config: %s\n", result.ConfigPath)
		fmt.Fprintln(cmd.OutOrStdout(), "")
		fmt.Fprintln(cmd.OutOrStdout(), "Next steps:")
		fmt.Fprintln(cmd.OutOrStdout(), "  Edit the configuration and set runtime.id:")
		fmt.Fprintf(cmd.OutOrStdout(), "    anvil server config set runtime.id <server-id>\n")
	}
