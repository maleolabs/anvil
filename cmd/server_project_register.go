...
func runServerProjectRegister(cmd *cobra.Command, args []string) error {
	...
	// 1. Check Runtime is initialized.
	...
	if err := registryStore.Register(project); err != nil {
		if strings.Contains(err.Error(), "project already registered") {
			fmt.Fprintf(cmd.ErrOrStderr(), "Error: %v.\n", err)
			return err
		}
	...
	if err := server.ProvisionProjectDir(project.Project.InstallRoot, project.Project.Owner, project.Project.Group); err != nil {
		fmt.Fprintf(cmd.ErrOrStderr(), "Warning: project registered but directory setup failed: %v.\n", err)
		fmt.Fprintf(cmd.ErrOrStderr(), "Directory structure may be incomplete. Check permissions and retry.\n")
		// Not a hard error — registry entry already created.
	}

	// 7. Success.
	projectPath := registryStore.ProjectPath(project.Project.ID)
	fmt.Fprintf(cmd.OutOrStdout(), "Project %s registered at %s\n", project.Project.ID, projectPath)
