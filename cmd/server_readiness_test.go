...
// that is not registered (and therefore not verified) is reported as an
// informational finding. The command is demoted, non-governing diagnostics
// (ADR-036 §3, TS-015-05-02): findings are reported, the exit code stays 0.
//
// Reference: ST-P9-02 (ST-009-002 AC3), ADR-036
func TestServerReadinessCommand_ArtifactUnverified(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("server", "readiness", projectID, releaseID, "--server-root", serverRoot)
	if err != nil {
		t.Fatalf("demoted readiness command must exit 0 even with findings, got error: %v", err)
	}
	...
	if stderr != "" {
		t.Errorf("demoted readiness command must not write a failure error, got stderr: %s", stderr)
	}
}

// TestServerReadinessCommand_ReleaseNotReady verifies that a Release not in
// the Ready stage is reported with the current stage and the requirement to
// reach Ready. The finding is informational — the exit code stays 0
// (ADR-036 §3, TS-015-05-02).
//
// Reference: ST-P9-02 (ST-009-002 AC4), ADR-036
...
// configuration is reported with a blocker identifying the specific key and
// expected format. The finding is informational — the exit code stays 0
// (ADR-036 §3, TS-015-05-02).
//
// Reference: ST-P9-02 (ST-009-002 AC2), ADR-036
...
// blockers and the command exits 0 (demoted, non-governing diagnostics —
// ADR-036 §3, TS-015-05-02).
//
// Reference: ST-P9-02, ADR-036
