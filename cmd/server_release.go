...
func runInstall(cmd *cobra.Command, args []string) error {
	...
	// Step 3: Validate the Runtime is initialized.
...
func runServerReleaseStatus(cmd *cobra.Command, args []string) error {
	...
	// Detail view: the stage of a single Release.
	if len(args) == 2 {
		releaseID := args[1]
		rel, err := release.LookupByID(installRoot, release.ReleaseID(releaseID))
		if err != nil {
...
var serverReleaseBuildCmd = &cobra.Command{
	...
	Short: "Build a release via the project's framework adapter",
	Long: `Run the project's framework adapter build pipeline on the server.

The build executes the adapter-owned build phases (e.g. for Laravel:
composer -> npm -> artisan caches) in the project install root, bounded
by a 15-minute timeout. The adapter's build phases are the single source
of build knowledge at server release time (ADR-020 §4).
...
The project must select a framework adapter (project.adapter in the
project registry); a missing adapter fails with a descriptive error —
there is no silent fallback to a generic build.
