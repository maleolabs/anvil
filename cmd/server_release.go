...
func runInstall(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(styleFor(cmd).W, "\nRelease created.\n")
	fmt.Fprintf(styleFor(cmd).W, "  Release ID: %s\n", rel.ID)
	fmt.Fprintf(styleFor(cmd).W, "  Artifact ID: %s\n", rel.ArtifactID)
	fmt.Fprintf(styleFor(cmd).W, "  Version: %s\n", rel.Version)
	fmt.Fprintf(styleFor(cmd).W, "  Stage: %s\n", rel.Stage)
	fmt.Fprintf(styleFor(cmd).W, "  Created: %s\n", rel.CreatedAt)
	fmt.Fprintln(styleFor(cmd).W, "")
	fmt.Fprintln(styleFor(cmd).W, "Next step: anvil server release activate <project-id> <release-id>")
...
func runHistory(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(styleFor(cmd).W, "Release History for %s\n", rel.ID)
	fmt.Fprintf(styleFor(cmd).W, "  Current Stage: %s\n", rel.Stage)
	fmt.Fprintln(styleFor(cmd).W, "")

	if len(history) == 0 {
		fmt.Fprintln(styleFor(cmd).W, "  No transitions recorded.")
		return nil
	}

	fmt.Fprintln(styleFor(cmd).W, "  Transitions:")
	for i, tr := range history {
		fmt.Fprintf(styleFor(cmd).W, "  %d. %s  %s \u2192 %s  %s\n",
...
func runActive(cmd *cobra.Command, args []string) error {
	...
	if rel == nil {
		...
		fmt.Fprintf(styleFor(cmd).W, "No Active Release.\n")
		fmt.Fprintln(styleFor(cmd).W, "")
		fmt.Fprintf(styleFor(cmd).W, "No Release is currently Active for project %q.\n", projectID)
		fmt.Fprintln(styleFor(cmd).W, "Install and activate a Release using:")
		fmt.Fprintf(styleFor(cmd).W, "  anvil server release install %s <artifact-path>\n", projectID)
		fmt.Fprintf(styleFor(cmd).W, "  anvil server release activate %s <release-id>\n", projectID)
		return nil
	}
	...
	// Human-readable output.
	fmt.Fprintf(styleFor(cmd).W, "Active Release\n")
...
func runServerReleaseStatus(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 2 {
		...
		fmt.Fprintf(styleFor(cmd).W, "Release Status for %s\n", rel.ID)
		PrintKeyValue(cmd, "Version", rel.Version)
		if rel.ArtifactID != "" {
			PrintKeyValue(cmd, "Artifact Reference", rel.ArtifactID)
		}
	...
	if len(releases) == 0 {
		fmt.Fprintf(styleFor(cmd).W, "No Releases found for project %q.\n", projectID)
		fmt.Fprintln(styleFor(cmd).W, "")
		fmt.Fprintf(styleFor(cmd).W, "Install a Release using:\n")
		fmt.Fprintf(styleFor(cmd).W, "  anvil server release install %s <artifact-path>\n", projectID)
		return nil
	}
...
func runServerReleaseBuild(cmd *cobra.Command, args []string) error {
	...
	for _, phase := range result.Phases {
		switch {
		case phase.Skipped:
			fmt.Fprintf(styleFor(cmd).W, "  %s — skipped (%s)\n", phase.Phase, phase.Warning)
		case phase.Success:
			fmt.Fprintf(styleFor(cmd).W, "  %s — ok\n", phase.Phase)
		default:
			fmt.Fprintf(styleFor(cmd).W, "  %s — failed: %s\n", phase.Phase, phase.Error)
		}
	}
	fmt.Fprintln(styleFor(cmd).W, "")
	fmt.Fprintln(styleFor(cmd).W, "Next step: anvil artifact package, then anvil server release install <project-id> <artifact-path>")
