...
func setupActivateEnvironment(t *testing.T, serverRoot, releaseID string) (projectID, releasePath string) {
	...
	// Ensure shared env gate passes (console build-once pre-condition).
	...
	if err := os.WriteFile(filepath.Join(sharedEnvDir, ".env"), []byte("APP_ENV=production\nAPP_KEY=base64:testkey1234567890\n"), 0644); err != nil {
...
func TestActivateCommand_ProjectNotRegistered(t *testing.T) {
	...
	// Initialize the server so the Runtime gate passes and the
	// coordinator reaches the registry (TS-019-03-02 F-01).
	...
	if !contains(stderr, "project registry not found") && !contains(stderr, "registry") {
		t.Errorf("expected a project-registry error in stderr, got: %s", stderr)
	}
}
