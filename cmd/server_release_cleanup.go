...
func runCleanup(cmd *cobra.Command, args []string) error {
	...
	// TS-019-03-02 F-01) — the gate runs before any registry work.
	if err := RequireServerInitialized(cmd, rootPath); err != nil {
		return err
	}

	// Step 1: Load the project registry to resolve the install root. A
	// project not found in the registry is a runtime not-found (exit 3,
	// F-02).
	registryStore := server.NewRegistryStore(rootPath)
	if !registryStore.Exists(projectID) {
		return reportProjectNotFoundError(cmd, projectID, fmt.Errorf("project %q not registered", projectID))
	}
	...
	// Step 3: Check if the release directory exists. A missing release
	// directory is a runtime not-found (exit 3, F-02).
	if _, err := os.Stat(releaseDir); err != nil {
		if os.IsNotExist(err) {
			return reportReleaseNotFoundError(cmd, projectID, releaseID,
				fmt.Errorf("release directory %s not found", releaseDir))
		}
	...
	if !force {
		...
		fmt.Fprintf(styleFor(cmd).W, "Warning: You are about to remove release '%s'.\n", releaseID)
		fmt.Fprintf(styleFor(cmd).W, "This will delete the release directory and all its contents.\n")
		fmt.Fprintf(styleFor(cmd).W, "Are you sure you want to remove release '%s'? This will delete the release directory and all its contents. [y/N] ", releaseID)
		...
		if response != "y" && response != "yes" {
			fmt.Fprintln(styleFor(cmd).W, "Cleanup cancelled.")
			return nil
		}
	}
	...
	fmt.Fprintf(styleFor(cmd).W, "  Space reclaimed: %s\n", formatBytes(size))
	fmt.Fprintln(styleFor(cmd).W, "")
	fmt.Fprintln(styleFor(cmd).W, "Other release directories and shared resources were not affected.")
