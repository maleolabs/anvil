...
func TestInstallCommand_NonexistentArtifact(t *testing.T) {
	...
	// Initialize the server so the artifact-not-found check is reached
	// (the Runtime gate runs first, TS-019-03-02 §9.3).
	if _, _, _, err := executeCommand("server", "init", "--server-root", serverRoot); err != nil {
...
// TestInstallCommand_InitCheckBeforeArtifact verifies that:
//
...
func TestInstallCommand_InitCheckBeforeArtifact(t *testing.T) {
	dir := t.TempDir()

	// Do NOT initialize the server. Trying to install a non-existent
	// artifact against an uninitialized Runtime must fail with the
	// init error (precondition), not the artifact error — the runtime
	// gate is the first check.
	_, _, stderr, err := executeCommand("server", "release", "install",
...
func TestInstallCommand_OutputFormat(t *testing.T) {
	...
	for _, line := range strings.Split(stdout, "\n") {
		...
		if strings.HasPrefix(trimmed, "Release ID: ") {
			releaseID = strings.TrimPrefix(trimmed, "Release ID: ")
			break
		}
	}
	if releaseID == "" {
...
// ── Release Status Command Tests (TS-012-002) ────────────────────────

// saveStatusTestRelease persists a Release in the given lifecycle stage
// into the releases state directory for status command tests.
func saveStatusTestRelease(t *testing.T, releaseDir, id, version string, stage release.Stage) {
	t.Helper()

	rel := &release.Release{
		ID:          release.ReleaseID(id),
		ArtifactID:  "test-artifact",
		Version:     version,
		Stage:       stage,
		CreatedAt:   "2024-01-15T10:00:00Z",
	...
	relPath := filepath.Join(releaseDir, id+".json")
	if err := rel.Save(relPath); err != nil {
		t.Fatalf("save release %s: %v", id, err)
	}
}

// TestStatusCommand_ListsAllReleasesWithStages verifies that:
//
//	anvil server release status <project-id> --server-root <root>
//
// lists every Release with its lifecycle stage.
//
// AC: The status command lists all releases with their lifecycle stage
// (ready/activating/active/rolled back/archived/failed).
//
// Reference: TS-012-002 AC-1
func TestStatusCommand_ListsAllReleasesWithStages(t *testing.T) {
	serverRoot, projectID, _, releaseDir := setupReadOnlyEnvironment(t)

	saveStatusTestRelease(t, releaseDir, "rel-ready", "1.0.0", release.StageReady)
	saveStatusTestRelease(t, releaseDir, "rel-active", "2.0.0", release.StageActive)
	saveStatusTestRelease(t, releaseDir, "rel-rolledback", "3.0.0", release.StageRolledBack)
	saveStatusTestRelease(t, releaseDir, "rel-archived", "4.0.0", release.StageArchived)
	saveStatusTestRelease(t, releaseDir, "rel-failed", "5.0.0", release.StageFailed)

	_, stdout, stderr, err := executeCommand("server", "release", "status",
		projectID, "--server-root", serverRoot)
	if err != nil {
		t.Fatalf("status command returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	// Every saved Release must be listed with its identity and stage.
	for _, id := range []string{"rel-ready", "rel-active", "rel-rolledback", "rel-archived", "rel-failed"} {
		if !contains(stdout, id) {
			t.Errorf("expected release %q in stdout, got: %s", id, stdout)
		}
	}
	for _, stage := range []string{"ready", "active", "rolled back", "archived", "failed"} {
		if !contains(stdout, stage) {
			t.Errorf("expected stage %q in stdout, got: %s", stage, stdout)
		}
	}
}

// TestStatusCommand_SingleReleaseDetail verifies that:
//
//	anvil server release status <project-id> <release-id> --server-root <root>
//
// shows the stage of the specified Release.
//
// AC: The status command shows the stage of the specified release.
//
// Reference: TS-012-002 AC-2
func TestStatusCommand_SingleReleaseDetail(t *testing.T) {
	serverRoot, projectID, _, releaseDir := setupReadOnlyEnvironment(t)
	...
	_, stdout, stderr, err := executeCommand("server", "release", "status",
		projectID, "rel-active", "--server-root", serverRoot)
	if err != nil {
		t.Fatalf("status command returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	if !contains(stdout, "Release Status for rel-active") {
		t.Errorf("expected release identity header in stdout, got: %s", stdout)
	}
	if !contains(stdout, "Stage: active") {
		t.Errorf("expected 'Stage: active' in stdout, got: %s", stdout)
	}
	...
	if !contains(stdout, "Created:") {
		t.Errorf("expected created timestamp in stdout, got: %s", stdout)
	}
}

// TestStatusCommand_UnknownRelease verifies that an unknown release ID
// produces an actionable error naming the release.
//
// AC: Missing or unknown release IDs produce actionable errors consistent
// with the other server release commands.
//
// Reference: TS-012-002 §7
func TestStatusCommand_UnknownRelease(t *testing.T) {
	serverRoot, projectID, _, _ := setupReadOnlyEnvironment(t)

	_, _, stderr, err := executeCommand("server", "release", "status",
		projectID, "nonexistent-release", "--server-root", serverRoot)
	if err == nil {
		t.Fatal("status command should return error for nonexistent release")
	}

	if !contains(stderr, "Release") || !contains(stderr, "not found") {
		t.Errorf("expected 'Release not found' in stderr, got: %s", stderr)
	}
	if !contains(stderr, "nonexistent-release") {
		t.Errorf("expected the release ID in the error, got: %s", stderr)
	}
}

// TestStatusCommand_UnknownProject verifies that an unregistered project
// fails with the same descriptive error as the other server release
// commands.
//
// Reference: TS-012-002 §7
func TestStatusCommand_UnknownProject(t *testing.T) {
	serverRoot := t.TempDir()
	...
	if err == nil {
		t.Fatal("status command should fail for an unregistered project")
	}
	if !contains(stderr, "not found in the Server Runtime Registry") {
		t.Errorf("expected project-not-found error in stderr, got: %s", stderr)
	}
}

// TestStatusCommand_JSONOutput verifies the --json flag on the list view
// produces valid machine-readable output carrying every Release stage.
//
// Reference: TS-012-002 AC-3
func TestStatusCommand_JSONOutput(t *testing.T) {
	...
	_, stdout, stderr, err := executeCommand("server", "release", "status",
		projectID, "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("status command with --json returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	if !contains(stdout, "\"project_id\": \"test-project\"") {
		t.Errorf("expected 'project_id' in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"releases\"") {
		t.Errorf("expected 'releases' array in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"release_id\": \"rel-ready\"") {
		t.Errorf("expected release_id in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"stage\": \"ready\"") {
		t.Errorf("expected stage in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"stage\": \"active\"") {
		t.Errorf("expected stage in JSON output, got: %s", stdout)
	}
...
// TestStatusCommand_SingleReleaseJSONOutput verifies the --json flag on
// the detail view produces the per-Release machine-readable shape.
//
// Reference: TS-012-002 AC-3
func TestStatusCommand_SingleReleaseJSONOutput(t *testing.T) {
	serverRoot, projectID, _, releaseDir := setupReadOnlyEnvironment(t)

	saveStatusTestRelease(t, releaseDir, "rel-active", "2.0.0", release.StageActive)

	_, stdout, stderr, err := executeCommand("server", "release", "status",
		projectID, "rel-active", "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("status command with --json returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	if !contains(stdout, "\"release_id\": \"rel-active\"") {
		t.Errorf("expected 'release_id' in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"stage\": \"active\"") {
		t.Errorf("expected 'stage' in JSON output, got: %s", stdout)
	...
	if !contains(stdout, "\"created_at\"") {
		t.Errorf("expected 'created_at' in JSON output, got: %s", stdout)
	}
}

// TestStatusCommand_NoReleases verifies the list view behavior when the
// project has no Releases yet.
//
// Reference: TS-012-002 §7
func TestStatusCommand_NoReleases(t *testing.T) {
	serverRoot, projectID, _, _ := setupReadOnlyEnvironment(t)

	// Human-readable mode reports no releases and points to install.
	_, stdout, stderr, err := executeCommand("server", "release", "status",
		projectID, "--server-root", serverRoot)
	if err != nil {
		t.Fatalf("status command returned unexpected error: %v\nstderr: %s", err, stderr)
	}
	if !contains(stdout, "No Releases found") {
		t.Errorf("expected 'No Releases found' in stdout, got: %s", stdout)
	}

	// JSON mode emits an empty releases array (never null).
	_, stdout, stderr, err = executeCommand("server", "release", "status",
		projectID, "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("status command with --json returned unexpected error: %v\nstderr: %s", err, stderr)
	...
	if !contains(stdout, "\"releases\": []") {
		t.Errorf("expected empty releases array in JSON output, got: %s", stdout)
	}
}

// TestStatusCommand_NoStateModification verifies that the status command
// does not modify any files.
//
// AC: The status query does not modify any files or state.
//
// Reference: TS-012-002 §3
func TestStatusCommand_NoStateModification(t *testing.T) {
	serverRoot, projectID, _, releaseDir := setupReadOnlyEnvironment(t)
	...
	executeCommand("server", "release", "status",
		projectID, "--server-root", serverRoot)
	executeCommand("server", "release", "status",
		projectID, "rel-active", "--server-root", serverRoot)
...
// TestStatusCommand_MissingArgs verifies that running without the project
// ID produces an arg validation error.
func TestStatusCommand_MissingArgs(t *testing.T) {
	_, _, stderr, err := executeCommand("server", "release", "status")
	if err == nil {
		t.Fatal("expected error for missing args, got nil")
	}
	if !contains(stderr, "requires between") {
		t.Errorf("expected arg validation error, got: %s", stderr)
	}

	// More than two args is also invalid.
	_, _, stderr, err = executeCommand("server", "release", "status", "p1", "r1", "extra")
	if err == nil {
...
// ── Server Release Build Command Tests (TS-007-040) ──────────────────

// setupBuildCommandEnvironment registers a project that selects
// adapterName and returns the project ID. The server root is a temp
// directory managed by the test.
func setupBuildCommandEnvironment(t *testing.T, serverRoot, adapterName string) (projectID string) {
	t.Helper()
	...
	// The build runs in the install root — create it so the adapter
...
// writeCLIBuildStubAdapter writes an executable stub adapter named
// "anvil-adapter-<framework>" into dir that answers the capabilities,
// extension, and build commands of the adapter contract. failPhase
// selects the failing build variant (empty = build succeeds). The stub
// uses only shell builtins so it works under a controlled PATH.
func writeCLIBuildStubAdapter(t *testing.T, dir, framework, failPhase string) string {
	t.Helper()
	...
	script := `#!/bin/sh
case "$1" in
  capabilities) echo '{"capabilities":{"build_phases":["composer","npm"]}}' ;;
...
`
	...
	path := filepath.Join(dir, "anvil-adapter-"+framework)
	if err := os.WriteFile(path, []byte(script), 0o755); err != nil {
		t.Fatalf("write build stub adapter: %v", err)
	}
	return path
}

// TestServerReleaseBuildCommand_Success verifies that:
//
//	anvil server release build <project-id> --server-root <root>
//
// invokes the installed framework adapter's build pipeline and reports
// the phase outcomes.
func TestServerReleaseBuildCommand_Success(t *testing.T) {
	serverRoot := t.TempDir()
	projectID := setupBuildCommandEnvironment(t, serverRoot, "laravel")

	adapterDir := t.TempDir()
	writeCLIBuildStubAdapter(t, adapterDir, "laravel", "")
	t.Setenv("PATH", adapterDir)

	_, stdout, stderr, err := executeCommand("server", "release", "build",
		projectID, "--server-root", serverRoot)
	if err != nil {
		t.Fatalf("server release build returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	if !contains(stdout, "Build complete") {
		t.Errorf("expected 'Build complete' in stdout, got: %s", stdout)
	}
	if !contains(stdout, "composer") {
		t.Errorf("expected phase 'composer' in stdout, got: %s", stdout)
	}
	if !contains(stdout, "npm") {
		t.Errorf("expected phase 'npm' in stdout, got: %s", stdout)
	}
}

// TestServerReleaseBuildCommand_JSONOutput verifies the --json flag
// produces machine-readable output carrying the build outcome and
// phases.
func TestServerReleaseBuildCommand_JSONOutput(t *testing.T) {
	serverRoot := t.TempDir()
	projectID := setupBuildCommandEnvironment(t, serverRoot, "laravel")
	...
	_, stdout, stderr, err := executeCommand("server", "release", "build",
		projectID, "--server-root", serverRoot, "--json")
	if err != nil {
		t.Fatalf("server release build --json returned unexpected error: %v\nstderr: %s", err, stderr)
	}

	if !contains(stdout, "\"success\": true") {
		t.Errorf("expected 'success: true' in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"phases\"") {
		t.Errorf("expected 'phases' in JSON output, got: %s", stdout)
	}
	if !contains(stdout, "\"phase\": \"composer\"") {
		t.Errorf("expected phase 'composer' in JSON output, got: %s", stdout)
	}
}

// TestServerReleaseBuildCommand_ProjectNotRegistered verifies that an
// unknown project fails with a descriptive error.
func TestServerReleaseBuildCommand_ProjectNotRegistered(t *testing.T) {
	serverRoot := t.TempDir()

	// Initialize the server config so the command reaches the project
	// registry lookup (no project is registered).
	configStore := server.NewConfigStore(serverRoot)
	cfg := server.DefaultServerConfig()
	cfg.Runtime.ID = "test-runtime"
	if err := configStore.Save(cfg); err != nil {
	...
	_, _, stderr, err := executeCommand("server", "release", "build",
		"nonexistent-project", "--server-root", serverRoot)
	if err == nil {
		t.Fatal("server release build should fail for an unregistered project")
	}
...
// TestServerReleaseBuildCommand_MissingAdapter verifies that a project
// selecting an adapter whose executable is not installed fails with a
// descriptive error naming the binary — no silent generic build.
func TestServerReleaseBuildCommand_MissingAdapter(t *testing.T) {
	serverRoot := t.TempDir()
	projectID := setupBuildCommandEnvironment(t, serverRoot, "no-such-framework-xyz")

	// No fake adapter on PATH: the convention resolver must fail.
	_, _, stderr, err := executeCommand("server", "release", "build",
		projectID, "--server-root", serverRoot)
	if err == nil {
		t.Fatal("server release build should fail for a missing adapter executable")
	}
	if !contains(stderr, "anvil-adapter-no-such-framework-xyz") {
		t.Errorf("expected the missing executable name in stderr, got: %s", stderr)
	}
}

// TestServerReleaseBuildCommand_FailingPhase verifies that a failing
// build phase fails the command with an error naming the phase and its
// output details.
func TestServerReleaseBuildCommand_FailingPhase(t *testing.T) {
	serverRoot := t.TempDir()
	projectID := setupBuildCommandEnvironment(t, serverRoot, "laravel")

	adapterDir := t.TempDir()
	writeCLIBuildStubAdapter(t, adapterDir, "laravel", "npm")
	t.Setenv("PATH", adapterDir)

	_, _, stderr, err := executeCommand("server", "release", "build",
		projectID, "--server-root", serverRoot)
	if err == nil {
		t.Fatal("server release build should fail when a build phase fails")
	}
	if !contains(stderr, `phase "npm"`) {
		t.Errorf("expected the failing phase in stderr, got: %s", stderr)
	}
	if !contains(stderr, "npm build failed on purpose") {
		t.Errorf("expected the failing phase's output details in stderr, got: %s", stderr)
	}
}

// TestServerReleaseBuildCommand_MissingArgs verifies that running
// without the project ID produces an arg validation error.
func TestServerReleaseBuildCommand_MissingArgs(t *testing.T) {
	_, _, stderr, err := executeCommand("server", "release", "build")
	if err == nil {
