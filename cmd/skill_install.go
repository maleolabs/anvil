...
Interactive mode (no name argument):
  Running 'anvil skill install' with no name on an interactive
  terminal opens a three-stage form: select skills (checkbox
  multi-select, space toggles, enter continues), select agents,
  select scope (repo/global). The selection runs through the same
  gated batch install path as --all (same pipeline, records, and
  targets — never a bypass). Ctrl-C cancels and restores the
  terminal; Esc aborts. When stdin is not a terminal the command
  fails with an actionable error — use 'anvil skill install <name>'
  or 'anvil skill install --all'. --json is the automation surface
  and never opens the form. --force does not apply to the form: the
  interactive install always runs without --force — for a conflict,
  run 'anvil skill install <name> --force'.
...
Examples:
  ...
  anvil skill install --all --scope global --agent opencode`,
	Args: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) > 1 {
			return fmt.Errorf("the %q command accepts at most 1 argument: <name>\nExample: anvil skill install anvil-overview", cmd.CommandPath())
		}
		return nil
	},
	SilenceUsage: true,
...
// runSkillInstall executes the install command: with no name argument on
// a terminal it opens the interactive form (ST-021-07); otherwise it
// resolves the skill (core or standard), runs the source's gates,
// materializes the targets, and records.
func runSkillInstall(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		return runSkillInstallInteractive(cmd)
	}
...
// agents and scope, with per-skill failure isolation. It resolves the
// --all inputs (flags + discovery) and delegates the install loop and
// reporting to runBatchSkillInstall — the same runner the interactive
// installer uses, so both modes share the gated pipeline.
func runSkillInstallAll(cmd *cobra.Command) error {
	...
	// Scope preconditions are checked BEFORE discovery (m4): a repo-scope
	// precondition (no Anvil project / no git root) is exit 4 even when
	// the available skill set would be empty — the scope problem is
	// diagnosed first, preserving the --all exit-code precedence. The
	// check also runs inside runBatchSkillInstall for the interactive
	// path, which reaches here through a different resolution flow.
	if _, serr := skillScopeBase(scope); serr != nil {
		return skillReportScopeError(cmd, fmt.Sprintf("cannot resolve the %s scope", scope), serr)
	}
	...
	return runBatchSkillInstall(cmd, names, agents, scope, force)
}
...
func installStandardSkill(cmd *cobra.Command, name string, scope agenttarget.Scope, agents []agenttarget.Agent, force bool) (*skillInstallResult, error) {
	...
	// anchors, fetch, verify digest, extract, install targets, record —
