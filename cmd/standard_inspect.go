...
the index (versions ordered semantically, 1.2.3 before 1.10.0), with
its declared contract version, capability, and lifecycle state. With a
version argument it displays the full detail of that release: declared
contract version, capability, distribution location, lifecycle state,
and trust presence.
...
Output formats:
  Default      sectioned detail for a pinned release; a rounded table
               (Version, Contract, Capability, Status) plus Warnings
               and Invalid entries sections for the overview
...
func renderStandardInspectOverview(cmd *cobra.Command, id string, views []standardEntryView) {
	...
	PrintRoundedTable(cmd, []string{"Version", "Contract", "Capability", "Status"}, rows)
	fmt.Fprintln(w)
