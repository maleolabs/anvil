...
// presence. The raw index-document path is intentionally absent from the
// human output (ST-021-05 — hints replace excessive raw paths; the
// source stays in the --json surface).
//
...
func TestStandardInspect_PinnedPublished(t *testing.T) {
	...
	if strings.Contains(stdout, "Index Document:") {
		t.Errorf("stdout should not render the raw index-document path section (ST-021-05), got:\n%s", stdout)
	}
}
