...
// side effect of another command. The command resolves the declared
// version from the static registry index, validates metadata, verifies
// integrity and attestation, records the installed version, and remains
// idempotent by standard identity plus version.
//
...
//     and the shared per-request timeout; the ACTUAL endpoint used — the
//     final response URL after any allowed redirects — is recorded as
//     the explicit resolution; a fetch failure aborts the install with
//     an actionable error;
...
//   - timeout: the shared timed HTTP client (60s default, overridable
//     via ANVIL_HTTP_TIMEOUT);
...
command resolves the declared version, validates the metadata, fetches
...
Examples:
  ...
  anvil standard install anvil-standard-laravel 1.2.3 --json`,
	Args:         RangeArgsWithUsage(2, 2, "anvil standard install anvil-standard-laravel 1.2.3", "id", "version"),
	SilenceUsage: true,
...
// adoption order (file header): resolve → strict parse → supported
// contract majors (compatibility matrix) → project framework version →
// adoption validation pre-fetch phase (lifecycle gate → compatibility) →
// location → fetch → adoption validation post-fetch phase (trust +
// combined record) → record. Both validation phases always run in a
// full adoption and a failure at any gate aborts the install with an
...
func runStandardInstall(cmd *cobra.Command, args []string) error {
	...
	// 1. Load the index and resolve the entry. This is structural decode
	// only (TS-014-02-01); the strict parse follows in step 2.
	...
	// 2. Strict parse (TS-014-01-02): the raw document is re-read from
	...
	if err != nil {
		return ReportError(cmd, &output.AppError{
			...
			Reason:     err.Error(),
			Resolution: "Fix or remove the metadata document in the index, then run the install again",
			Err:        err,
		})
	}
	...
	// 4. Project framework version gate (PM binding decision 3): the
	...
	// 5. Adoption validation, pre-fetch phase (TS-014-04-03): inside the
	...
	// 6. Content location resolution (TS-014-02-03). Resolution
	...
	// 7. Trust anchors BEFORE the fetch (reviewer finding 2): resolving
	...
	// 8. Fetch the release content (ADR-030): https-only, bounded
	...
	// 9. Adoption validation, post-fetch phase (TS-014-04-03): trust
	...
	// 10. Record the installed version (TS-014-03-03): pinned version,
	...
	return recordStandardInstall(cmd, *md, contentSource, adoption, parseWarnings)
}
...
// persistence shape (T-009).
func recordStandardInstall(cmd *cobra.Command, md registry.Metadata, contentSource string, adoption registry.AdoptionResult, parseWarnings []registry.Warning) error {
