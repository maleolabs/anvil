...
func TestStandardInstall_RecordRejectsNilTrust(t *testing.T) {
	...
	err := recordStandardInstall(&cobra.Command{}, md, "https://example.com/release.tar.gz", before, nil, false)
	if err == nil {
...
func TestStandardInstall_NoAnchors(t *testing.T) {
	...
	t.Run("empty allowlist fails fast with the actionable no-anchor message", func(t *testing.T) {
		...
		if fetchCount != before {
			t.Errorf("content fetched %d time(s) with an empty anchors allowlist, want 0 — anchors must gate before the fetch", fetchCount-before)
		}
		if _, err := os.Stat(installTestRecordPath(t, id)); !os.IsNotExist(err) {
...
// TestStandardInstall_FetchTimeout verifies that a stalled content
// download surfaces an explicit timeout error (shared download client,
// TD-008 §4). The server completes the header phase and then stalls
// MID-BODY — the failure mode that matters for large downloads: a total
// per-request deadline would cut off slow-but-progressing transfers, so
// the body read is bounded by the idle window instead, and a transfer
// that stops delivering data must still fail within it.
//
...
func TestStandardInstall_FetchTimeout(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Complete the header phase, flush one byte, then stall: the
		// fetch must fail on the idle-timeout body bound, not on a
	...
	installTestEnv(t, server)
	// Shrink the download idle timeout so the test does not wait on the
	// production default stall window (installTestEnv already rebuilds
	// standardInstallHTTPClient with a transport trusting the test
...
// installTestVersionSpec describes one version of a standard for the
// multi-version latest-resolution fixtures.
type installTestVersionSpec struct {
	State       string
	RemovalDate string
}

// installTestMultiVersionIndex assembles a fixture index with one
// standard at several versions — each a fully attested release over the
// same shared content served by a local https server — and the publisher
// anchors file. It returns the index directory and the anchors file; the
// command environment is isolated (installTestEnv).
func installTestMultiVersionIndex(t *testing.T, id string, versions map[string]installTestVersionSpec) (indexDir, anchorsFile string) {
	t.Helper()
	pub, priv := installTestKeypair(t)
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write(installTestStandardContent(id))
	}))
	...
	for version, spec := range versions {
		md := installTestRelease(t, id, version, server.URL+"/release.tar.gz",
			spec.State, spec.RemovalDate, []string{"5.1.0"}, installTestStandardContent(id), pub, priv)
		installTestIndexEntry(t, indexDir, md)
	}
	return indexDir, anchorsFile
}

// TestStandardInstall_LatestResolution_SemverOrdering verifies that
// "anvil standard install <id>" (version omitted) resolves the latest
// published release with semver ordering: 2.0.0 beats 1.10.0 beats
// 1.2.3. The resolved version is pinned and recorded; the human report
// annotates the automated choice.
//
// Reference: ST-021-05, ADR-022 §3, ADR-023 §3
func TestStandardInstall_LatestResolution_SemverOrdering(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.2.3":  {State: registry.LifecycleStatePublished},
		"1.10.0": {State: registry.LifecycleStatePublished},
		"2.0.0":  {State: registry.LifecycleStatePublished},
	})

	_, stdout, stderr, err := executeCommand("standard", "install", id, "--index", indexDir, "--trust-anchors", anchorsFile)
	if err != nil {
		t.Fatalf("install without a version failed: %v (stderr: %q)", err, stderr)
	}
	if !strings.Contains(stdout, "Installed standard: "+id+" 2.0.0 (latest published release)") {
		t.Errorf("stdout should report the resolved latest release, got:\n%s", stdout)
	}
	rec := installTestReadRecord(t, id)
	if rec.Version != "2.0.0" {
		t.Errorf("record version = %s, want the latest 2.0.0", rec.Version)
	}
	if rec.Lifecycle.State != registry.LifecycleStatePublished {
		t.Errorf("record lifecycle = %q, want published", rec.Lifecycle.State)
	}
...
// TestStandardInstall_LatestResolution_RetiredExcluded verifies that a
// retired release is never the latest-resolution target (ADR-027 §3):
// with 3.0.0 retired and 2.0.0 published, the resolution skips the
// retired 3.0.0 and adopts 2.0.0.
//
// Reference: ST-021-05, ADR-027 §3
func TestStandardInstall_LatestResolution_RetiredExcluded(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.0.0": {State: registry.LifecycleStatePublished},
		"2.0.0": {State: registry.LifecycleStatePublished},
		"3.0.0": {State: registry.LifecycleStateRetired},
	})
	...
	if !strings.Contains(stdout, "2.0.0 (latest published release)") {
		t.Errorf("stdout should resolve past the retired 3.0.0 to 2.0.0, got:\n%s", stdout)
	}
	rec := installTestReadRecord(t, id)
	if rec.Version != "2.0.0" {
		t.Errorf("record version = %s, want 2.0.0 (retired 3.0.0 excluded)", rec.Version)
	}
}

// TestStandardInstall_LatestResolution_AllRetired verifies the exhausted
// case: a standard whose every version is retired has no resolvable
// release — the install fails with an actionable error (exit 1) and
// nothing is recorded.
//
// Reference: ST-021-05, ADR-027 §3
func TestStandardInstall_LatestResolution_AllRetired(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.0.0": {State: registry.LifecycleStateRetired},
		"2.0.0": {State: registry.LifecycleStateRetired},
	})

	_, _, stderr, err := executeCommand("standard", "install", id, "--index", indexDir, "--trust-anchors", anchorsFile)
	requireExitCode(t, err, output.ExitCodeGeneral)
	if !strings.Contains(stderr, "no release offered for adoption") {
		t.Errorf("stderr missing the no-adoptable-release message: %q", stderr)
	}
	if _, err := os.Stat(installTestRecordPath(t, id)); !os.IsNotExist(err) {
		t.Errorf("record exists after an all-retired resolution, want nothing recorded")
	}
}

// TestStandardInstall_LatestResolution_UnknownStandard verifies the
// not-found contract of a version-less install: a standard absent from
// the index is a not-found failure (exit 3, TS-P8-07).
//
// Reference: ST-021-05
func TestStandardInstall_LatestResolution_UnknownStandard(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.2.3": {State: registry.LifecycleStatePublished},
	})

	_, _, stderr, err := executeCommand("standard", "install", "anvil-standard-missing", "--index", indexDir, "--trust-anchors", anchorsFile)
	requireExitCode(t, err, output.ExitCodeRuntime)
	if !strings.Contains(stderr, "not found") {
...
// TestStandardInstall_LatestResolution_EmptyIndex verifies the empty
// index: a version-less install against an index with no documents finds
// no standard — not-found (exit 3).
//
// Reference: ST-021-05
func TestStandardInstall_LatestResolution_EmptyIndex(t *testing.T) {
	const id = "anvil-standard-laravel"
	pub, _ := installTestKeypair(t)
	installTestEnv(t, nil)
	indexDir := t.TempDir() // empty index
	anchorsFile := installTestAnchorsFile(t, t.TempDir(), id, pub)

	_, _, stderr, err := executeCommand("standard", "install", id, "--index", indexDir, "--trust-anchors", anchorsFile)
	requireExitCode(t, err, output.ExitCodeRuntime)
	if !strings.Contains(stderr, "not found") {
		t.Errorf("stderr missing the not-found message for an empty index: %q", stderr)
	}
}

// TestStandardInstall_LatestResolution_InvalidTopFails verifies that an
// invalid release at the top of the version ladder fails the resolution
// with the actionable validation problem (exit 1) — the invalid release
// is never silently skipped in favor of an older version, and nothing is
// recorded.
//
// Reference: ST-021-05, TS-014-01-02
func TestStandardInstall_LatestResolution_InvalidTopFails(t *testing.T) {
	const id = "anvil-standard-laravel"
	pub, priv := installTestKeypair(t)
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write(installTestStandardContent(id))
	...
	indexDir := t.TempDir()
	anchorsFile := installTestAnchorsFile(t, t.TempDir(), id, pub)
	installTestIndexEntry(t, indexDir, installTestRelease(t, id, "1.0.0", server.URL+"/release.tar.gz",
		registry.LifecycleStatePublished, "", []string{"5.1.0"}, installTestStandardContent(id), pub, priv))
	// 2.0.0 is the newest declared release but fails strict validation
	// (lifecycle enum).
	installTestIndexEntry(t, indexDir, installTestRelease(t, id, "2.0.0", server.URL+"/release.tar.gz",
		"unknown-state", "", []string{"5.1.0"}, installTestStandardContent(id), pub, priv))

	_, _, stderr, err := executeCommand("standard", "install", id, "--index", indexDir, "--trust-anchors", anchorsFile)
	requireExitCode(t, err, output.ExitCodeGeneral)
	if !strings.Contains(stderr, "is invalid") {
		t.Errorf("stderr missing the invalid-latest message: %q", stderr)
	}
	if _, err := os.Stat(installTestRecordPath(t, id)); !os.IsNotExist(err) {
		t.Errorf("record exists after an invalid-latest resolution, want nothing recorded")
	}
}

// TestStandardInstall_LatestResolution_DeprecatedIsAdoptable verifies
// that a deprecated release is a valid latest-resolution target (only
// retired releases are excluded, ADR-027 §3): the deprecated 2.0.0 wins
// over the published 1.0.0, installs WITH the deprecation warning, and
// the record keeps the deprecated lifecycle state — the ADR-023 gates are
// unchanged.
//
// Reference: ST-021-05, ADR-023 §3, ADR-027 §3
func TestStandardInstall_LatestResolution_DeprecatedIsAdoptable(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.0.0": {State: registry.LifecycleStatePublished},
		"2.0.0": {State: registry.LifecycleStateDeprecated, RemovalDate: "2027-01-01T00:00:00Z"},
	})
	...
	if !strings.Contains(stdout, "2.0.0 (latest published release)") {
		t.Errorf("stdout should resolve the deprecated 2.0.0 as latest, got:\n%s", stdout)
	}
	if !strings.Contains(stdout, "deprecated") || !strings.Contains(stdout, "2027-01-01T00:00:00Z") {
		t.Errorf("stdout missing the deprecation warning for the resolved latest, got:\n%s", stdout)
	}
	rec := installTestReadRecord(t, id)
	if rec.Version != "2.0.0" || rec.Lifecycle.State != registry.LifecycleStateDeprecated {
		t.Errorf("record = %s (%s), want 2.0.0 deprecated", rec.Version, rec.Lifecycle.State)
	}
}

// TestStandardInstall_LatestResolution_Idempotent verifies that a
// version-less re-install of the same standard still runs the full
// validation and reports the already-installed state when the resolved
// latest equals the installed version.
//
// Reference: ST-021-05, TS-014-03-01 (PM binding decision 6)
func TestStandardInstall_LatestResolution_Idempotent(t *testing.T) {
	const id = "anvil-standard-laravel"
	indexDir, anchorsFile := installTestMultiVersionIndex(t, id, map[string]installTestVersionSpec{
		"1.2.3": {State: registry.LifecycleStatePublished},
		"2.0.0": {State: registry.LifecycleStatePublished},
	...
	if _, _, stderr, err := executeCommand("standard", "install", id, "--index", indexDir, "--trust-anchors", anchorsFile); err != nil {
		t.Fatalf("first version-less install failed: %v (stderr: %q)", err, stderr)
	}
	first := installTestReadRecord(t, id)
	if first.Version != "2.0.0" {
		t.Fatalf("first install version = %s, want 2.0.0", first.Version)
	}
	...
	if !strings.Contains(stdout, "already installed") {
		t.Errorf("stdout missing the already-installed report:\n%s", stdout)
	}
	second := installTestReadRecord(t, id)
	if !second.InstalledAt.Equal(first.InstalledAt) {
		t.Errorf("installedAt changed across a version-less re-install: %v → %v", first.InstalledAt, second.InstalledAt)
	}
}

// ── Missing-Index Degradation (ST-021-05) ────────────────────────────

// TestStandardInstall_MissingIndex_ConcreteHint verifies that a missing
// index still hard-fails the install — resolution requires the index —
// and the error carries a concrete first-run hint naming the --index
// flag, the ANVIL_REGISTRY_INDEX environment variable, and the fact that
// there is no bundled index (ADR-030).
//
// Reference: ST-021-05, ADR-030
func TestStandardInstall_MissingIndex_ConcreteHint(t *testing.T) {
	const id = "anvil-standard-laravel"
	installTestEnv(t, nil)
	missing := t.TempDir() + "/does-not-exist"

	_, _, stderr, err := executeCommand("standard", "install", id, "1.2.3", "--index", missing)
	requireExitCode(t, err, output.ExitCodeRuntime)
...
// TestStandardInstall_Args_OptionalVersion verifies the argument
// validator accepts a version-less install and rejects an argument-less
// one with the range explained.
//
// Reference: ST-021-05
func TestStandardInstall_Args_OptionalVersion(t *testing.T) {
	_, _, stderr, err := executeCommand("standard", "install")
	if err == nil {
		t.Fatal("install with no arguments must fail the argument validator")
	}
