...
// failure with actionable guidance about how the index path is chosen.
// A missing index directory is a not-found failure (exit 3,
...
func reportStandardIndexError(cmd *cobra.Command, err error) error {
	...
	return ReportErrorWithCode(cmd, &output.AppError{
		...
		Reason:     err.Error(),
		Resolution: "Set the index directory with --index <path> or the " + envStandardIndex + " environment variable (default: " + defaultStandardIndexDescription() + ")",
		Err:        err,
	}, exitCode)
}
...
// standardContractCell renders the contract-version column value, or "-"
// when none is declared (invalid entries carry no trustworthy contract
// version).
func standardContractCell(view standardEntryView) string {
	if view.ContractVersion == "" {
		return "-"
	}
	return view.ContractVersion
}
...
// content fetches (shared by the install and update flows): the shared
// per-request timeout (httpClient — 60s default, ANVIL_HTTP_TIMEOUT
// override) plus a redirect policy that never follows a redirect away
// from https. It is a package-level seam: tests swap it with a client
// that trusts the local test server.
...
// download via a limit reader, and the shared per-request timeout. It
// returns the content and the ACTUAL endpoint used — the final response
// URL after any allowed redirects — which the caller records as the
// explicit resolution (ADR-022 §3). A failure — 404, 5xx, DNS, timeout,
// TLS — returns an actionable error naming what failed; the caller
// aborts the adoption, so no content is adopted on failure.
func fetchStandardContent(location string) ([]byte, string, error) {
	...
	if err != nil {
		...
		return nil, "", fmt.Errorf(
			...
			location, httpError(standardScrubURLError(err)))
	}
