...
func TestStandardDomainGroup_HelpOutput(t *testing.T) {
	...
	// Scan the Development section. Robust to the output container's
	// uniform left margin (indent width may vary).
	...
	for _, rawLine := range lines {
		...
		if section == "Development" && strings.Contains(trimmed, "standard") {
			found = true
		}
	}
	if !found {
