...
// target version is the explicit version when given, otherwise the
// latest published release resolved from the index (ST-021-05); once
// resolved, the version is pinned per ADR-022 §3.
//
...
implicitly and never as a side effect of another command; there is no
auto-update path for server-side standards. When <version> is omitted,
the latest published release is resolved from the index (semver
ordering; retired releases are never resolved, ADR-027 §3) and becomes
the target. The resolved version is pinned exactly like an explicit one
(ADR-022 §3). ANY explicit version is a valid adoption event, including
...
Examples:
  anvil standard update anvil-standard-laravel
  ...
  anvil standard update anvil-standard-laravel 1.3.0 --json`,
	Args:         RangeArgsWithUsage(1, 2, "anvil standard update anvil-standard-laravel [version]", "id", "version"),
	SilenceUsage: true,
...
type standardUpdateResult struct {
	...
	RecordPath       string
	// ResolvedLatest marks a target resolved as the latest published
	// release (version omitted, ST-021-05) — the human report annotates
	// the choice; the JSON contract is unchanged.
	ResolvedLatest bool
	// DowngradeWarning carries the prominent warning for a version-less
	// update whose resolved latest is OLDER than the installed version
	// (reviewer M1, ST-021-05): the downgrade proceeds — it is a
	// documented adoption event — but never silently.
...
// resolve (with the strict parse — TS-014-01-02; a latest-resolution
// downgrade vs the installed version surfaces a prominent warning) →
// supported contract majors (compatibility matrix) → project framework
// version → adoption validation pre-fetch phase (lifecycle gate →
// compatibility) → location → trust anchors → fetch → adoption
// validation post-fetch phase (trust + combined record) → record update.
// Both validation phases always run in a full adoption, and a failure at
// any gate aborts the update with an actionable error and no state is
// written.
//
...
func runStandardUpdate(cmd *cobra.Command, args []string) error {
	...
	// 3. Load the index and resolve the target: the explicit version when
	...
	target, err := resolveStandardTarget(cmd, ix, id, version, "update")
	if err != nil {
		return err
	}
	...
	// Downgrade guard for the latest-resolution path (reviewer M1,
	// ST-021-05): when the version is omitted, the resolved latest can
	// be OLDER than the installed version — e.g. the installed release
	// was retired in the index, so the newest resolvable release is a
	// lower version. The update proceeds (an explicit downgrade is a
	// documented adoption event, TS-014-03-02) but the choice is never
	// silent: the report carries a prominent warning naming both
	// versions and the explicit-pin alternative.
	downgradeWarning := ""
	if resolvedLatest && standardVersionLess(version, existing.Version) {
		downgradeWarning = fmt.Sprintf(
			"the resolved target version %s is older than the installed version %s — this is a downgrade. To adopt a specific release instead, pin it explicitly: 'anvil standard update %s <version>'",
			version, existing.Version, id)
	}

	// 4. Supported contract majors from the compatibility matrix
	...
	// 5. Project framework version gate (PM binding decision 3): the
	...
	// 6. Adoption validation, pre-fetch phase (TS-014-04-03) on the
	...
	// 7. Content location resolution (TS-014-02-03). Resolution
	...
	// 8. Trust anchors BEFORE the fetch (same order as install): a
	...
	// 9. Fetch the release content (ADR-030): the SAME fetch boundary
	...
	// 10. Adoption validation, post-fetch phase (TS-014-04-03): trust
	...
	// 11. Record the update (TS-014-03-03): atomic replace with the new
	...
	// intact on failure.
	return recordStandardUpdate(cmd, target.Metadata, existing, contentSource, adoption, target.ParseWarnings, resolvedLatest, downgradeWarning)
}
...
func recordStandardUpdate(cmd *cobra.Command, md registry.Metadata, existing registry.InstalledStandardRecord, contentSource string, adoption registry.AdoptionResult, parseWarnings []registry.Warning, resolvedLatest bool, downgradeWarning string) error {
	...
	rec := registry.InstalledStandardRecord{
		...
		// The TARGET release's declared per-skill assets, refreshed at
		// the explicit re-validated update (ST-021-04 / ADR-037 D3): the
		// update replaces the record, so the declarations are replaced
		// with the target's — the record IS the skill registry.
...
func renderStandardUpdate(cmd *cobra.Command, result standardUpdateResult) {
	...
	if result.AlreadyAtVersion {
		...
		fmt.Fprintf(w, "Updated standard: %s %s (latest published release)\n", result.ID, result.Version)
	} else {
		fmt.Fprintf(w, "Updated standard: %s %s\n", result.ID, result.Version)
	}
	// The latest-resolution downgrade warning is rendered prominently,
	// directly under the header — the choice is never silent (reviewer
	// M1, ST-021-05).
	if result.DowngradeWarning != "" {
		fmt.Fprintf(w, "Warning: %s\n", result.DowngradeWarning)
	}
	fmt.Fprintln(w)
