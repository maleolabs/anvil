...
//     download, the shared per-request timeout — the SAME fetch boundary
//     as the install (reused fetchStandardContent; no --skip-verify and
//     no downgrade of trust); the ACTUAL endpoint used — the final
//     response URL after any allowed redirects — is recorded as the new
//     explicit resolution; a fetch failure aborts the update with an
//     actionable error and the old record stays intact;
