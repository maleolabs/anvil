...
func TestStandardUpdate_FetchFailures(t *testing.T) {
	...
	t.Run("timeout", func(t *testing.T) {
		...
		indexDir, anchorsFile, _, keys, _ := updateTestInstallBase(t, server)
		// Shrink the shared client timeout; the fetch client is rebuilt
		// from the shared timeout while keeping the transport that
		// trusts the test server's TLS certificate.
		origHTTP := httpClient
