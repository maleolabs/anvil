...
// artifact presence, shared links).
//
...
func runInspectRelease(cmd *cobra.Command, args []string) error {
	...
	if serverRoot != "" {
		checks = append(checks, inspector.InspectSharedLinks(serverRoot))
	}
	...
	appErr := &output.AppError{
		...
		Resolution: "Review the failed checks above and resolve each one; run 'anvil system diagnose' for recommendations referencing the owning Epic.",
	}
