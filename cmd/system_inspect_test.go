...
func TestSystemInspectRelease_Healthy(t *testing.T) {
	...
	// The shared-links check is removed with the demoted model (ADR-031 §3,
	// TS-015-04-02).
	if contains(stdout, "shared_links") {
		t.Errorf("stdout must not report the demoted shared_links check, got: %s", stdout)
	}
...
// does not exist reports a clear error with the runtime not-found category
// (exit 3, TS-019-03-02 F-02).
//
