...
//
// The "project remove" and "project version" surfaces were originally
// removed with this batch but are RESTORED (revert of the removal for
// the project group): the project command family — remove, version
// (set / bump:patch / bump:minor / bump:major / generate), and status —
// is a supported surface of the CLI, so those commands resolve again and
...
// (the removal announcement). The "project remove" / "project version"
// surfaces are intentionally absent: the project command family was
// restored, so those invocations resolve again.
...
// the parent groups' help (anvil artifact --help) no longer documents
// the removed subcommands.
//
