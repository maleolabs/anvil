...
(anvil-adapter-<name>) are refreshed to the same release version;
...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	// ── Adapter sync (TS-007-036) ──
	// Best-effort: refresh adapter binaries already installed next to the
	// CLI to the same release version. Nothing new is installed, failures
	...
	syncInstalledAdapters(adapterBinaryOps, reporter, execPath)
...
// replaceBinaryFallback installs tmpPath at targetPath by removing the old
// directory entry and creating a new file (cross-device rename fallback).
//
...
// filesystem than the install directory. Removing a non-existent file would
// otherwise abort the install with "remove old binary: no such file or
// directory".
func replaceBinaryFallback(tmpPath, targetPath string) error {
	// Remove old entry, then create new file.
