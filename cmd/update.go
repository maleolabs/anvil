...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	if checkOnly {
		...
		fmt.Fprintln(styleFor(cmd).W, "Run 'anvil update' to apply.")
		return nil
	}
	...
	// The download goes through the download client (httpGetDownload):
	// no total deadline that a slow-but-progressing download would trip
	// (a 13 MB binary on a slow link regularly exceeds the 60s metadata
	// timeout — "context deadline exceeded ... while reading body"),
	// but a stall surfaces within ANVIL_DOWNLOAD_IDLE_TIMEOUT (30s
	// default) as a clear timeout error (TD-008).
	resp, err := httpGetDownload(downloadURL)
	if err != nil {
