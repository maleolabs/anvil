...
// an upload test. Previous values are restored via t.Cleanup.
...
func unsetUploadEnv(t *testing.T) {
	t.Helper()
	for _, name := range []string{envDeployServerHost, envDeployServerUser, envDeployServerPort, envDeploySSHKey} {
		previous, existed := os.LookupEnv(name)
