...
| `skills` | no | array of skill objects | Optional additive skills declaration: the standard's per-skill assets (§4.8) |

Optional annotation fields follow the corpus convention: `$schema` (the `$id` of the schema this document targets), `title`, and `description` (human-readable annotations). They carry no validation semantics. The `skills` section (§4.8) is different from the annotations: it is an optional **section with validation semantics** — additive-only by design, but strictly validated when present (TS-021-04).

The document is minimal by design: every field in §4.1–§4.7 is required because every one of them is needed at adoption — identity and version (idempotent, pinned installation), contract version and capability (compatibility validation), distribution location (content resolution), lifecycle state (availability semantics), and trust fields (integrity and attestation). The only optional section is `skills` (§4.8), introduced additively for the AI-agent-skills distribution surface (ADR-037 D2) — a release without it behaves exactly as before.
...
### 4.8 Skills declaration: `skills` (optional additive section)

The optional `skills` section declares the standard's **per-skill assets** for the AI-agent-skills distribution surface (ADR-037 D2): standard skills are packed per-skill assets in the standard's release channel, declared in the registry metadata, and covered by the attested named-asset digests (TS-014-04-04; §4.7). Each skill's `asset` is the **safe identifier** of the release asset carrying the skill content (e.g. `anvil-skill-overview-1-0-0`); the asset name follows the named-digest name pattern of §4.7 — lowercase alphanumeric with hyphens, no dots or slashes — so it can never escape the release channel as a path component. The physical file in the release channel (e.g. `anvil-skill-<name>-<version>.tar.gz` per ADR-037 D2) is bound to the identifier by the release pipeline; the metadata binding and the install gate work on the safe identifier. The section is **additive-only and optional** — a release without it is valid and behaves exactly as before.

| Field | Required | Type | Meaning |
|---|---|---|---|
| `skills` | no | array of skill objects | Per-skill asset declarations of this release; each entry carries `name`, `version`, `asset`, and an optional `description` |

Each skill entry is a strict object:

| Field | Required | Type | Meaning |
|---|---|---|---|
| `name` | yes | string (safe identifier) | Skill name: the install target of `anvil skill install` and the namespace component of `skills/<standard-id>/<name>` (ADR-037 §7). Pattern `^[a-z0-9][a-z0-9-]*$` (corpus id convention), at most 64 characters — the pattern rejects traversal-style separators (dots, slashes, path separators), so a name can never escape its target location as a path component. Names are unique within one release |
| `version` | yes | string (semver) | Skill version, semver (major.minor.patch) without leading zeros — skills are versioned content; the version is part of the per-skill asset name and the recorded identity in the installed-skills store |
| `asset` | yes | string (safe asset identifier) | The safe identifier of the release asset carrying the skill content (e.g. `anvil-skill-overview-1-0-0`; the physical file, e.g. `anvil-skill-<name>-<version>.tar.gz`, is bound to the identifier by the release pipeline). Pattern `^[a-z0-9][a-z0-9-]*$`, at most 128 characters. **The asset must be covered by an attested named digest entry in `trust.contentDigests`** (§4.7, TS-014-04-04) — the asset binding makes the skill content attestation-bound like every other named release asset (ADR-037 D4: the install gate verifies the downloaded asset against the attested digest, fail-closed) |
| `description` | no | string | Optional human-readable description of the skill (what the agent gains by loading it); advisory annotation |

**Parser-enforced rules the schema cannot express.** Draft-07 cannot express per-property uniqueness over array items or an array cross-reference, so two rules are enforced by the registry client parser at parse time (the same precedent as the §4.7 `contentDigests` name-uniqueness rule, which is also schema-silent): (1) skill names are unique within one release — one release declares each skill name at most once; (2) every `asset` declared in `skills` is bound to a named `trust.contentDigests` entry — an undeclared or unbound asset is rejected with an actionable error.

**Forward-compatibility parser decision (PM-approved, recorded; TS-021-04; ADR-037 §6; ADR-024 spirit).** The `skills` extension is **additive-only**: it introduces one optional section and changes nothing about the required fields, their types, their constraints, or the trust semantics — the contract major is the compatibility unit and this extension stays within contract major 1 (see [compatibility-matrix](compatibility-matrix.md)). The parser tolerance posture for the deprecation window:

- **Older parsers accept and ignore unknown-but-optional sections.** A parser that does not know a newer additive section must not reject an otherwise-valid document: unknown root-level sections (object- or array-valued keys the schema does not declare) are accepted and ignored within the deprecation window.
- **Core fields stay strict.** Unknown root keys with scalar values remain rejected — the runtime-agnostic requirement stays enforceable (a runtime-typed scalar field still fails) — and `additionalProperties: false` inside every declared section is unchanged.
- **Declared sections stay strict.** A parser that DOES know the `skills` section validates it fully: a malformed skill declaration (invalid name, missing asset, invalid or unbound digest) is rejected with an actionable error, never silently accepted.
- The tolerance is scoped to the deprecation window of ADR-024 §3.4; a schema change that breaks compatibility remains a governed event requiring an ADR and a Core major version (§2.4, §6).

The decision is recorded here and reflected machine-readably in [registry-metadata.schema.json](registry-metadata.schema.json) (root `additionalProperties` tolerates unknown object/array sections; the `skill` definition carries the strict shape).
...
| §4 Document structure | ADR-021 §3.2; ADR-022 §3; ADR-023 §3; ADR-024 §3.1; ADR-027 §3; ADR-030 §3; ADR-037 D2 (skills); PRD-002 §5.7–§5.8; TS-014-04-04 (named asset digests) |
