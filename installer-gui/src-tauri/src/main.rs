...
fn apply_setup(app: tauri::AppHandle, forms: Value, dest_dir: String) -> Result<String, String> {
    ...
    let env_map = setup.get("env_map").and_then(Value::as_object).cloned().unwrap_or_default();
    let root = PathBuf::from(dest_dir);
    if !root.is_absolute() { return Err("install path must be absolute".into()); }
    let env_path = root.join(setup.get("env_file").and_then(Value::as_str).unwrap_or(".env"));
    let mut output = if env_path.exists() { fs::read_to_string(&env_path).map_err(|e| e.to_string())? } else { String::new() };
    for (key, template) in env_map {
        let mut value = template.as_str().unwrap_or_default().to_string();
        if let Some(inner) = value.strip_prefix("{{forms.").and_then(|v| v.strip_suffix("}}")) {
            let mut parts = inner.splitn(2, '.');
            let form = parts.next().unwrap_or_default();
            let field = parts.next().unwrap_or_default();
            value = forms.get(form).and_then(|v| v.get(field)).and_then(Value::as_str).unwrap_or_default().to_string();
        }
        let prefix = format!("{key}=");
        let line = format!("{key}={value}");
        let mut lines: Vec<String> = output.lines().map(str::to_owned).collect();
        if let Some(pos) = lines.iter().position(|line| line.starts_with(&prefix)) { lines[pos] = line; } else { lines.push(line); }
        output = lines.join("\n") + "\n";
    }
    if let Some(parent) = env_path.parent() { fs::create_dir_all(parent).map_err(|e| e.to_string())?; }
