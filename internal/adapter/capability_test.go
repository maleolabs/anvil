...
func TestCapabilityRegistry_RegisterEmptyDeclarationAllowed(t *testing.T) {
	...
	if len(decl.ActivationPhases) != 0 || len(decl.VerificationChecks) != 0 || len(decl.DiagnosticCommands) != 0 || len(decl.BuildPhases) != 0 || decl.DeploymentModel != "" {
		t.Errorf("Capabilities(\"laravel\") = %#v, want empty declaration", decl)
	}
}

// TestCapabilityRegistry_RegisterKnownDeploymentModels verifies that each
// known deployment model (server/hybrid/package) and an empty model are
// accepted when registering a declaration (TS-P7-13 AC-2).
//
// Reference: TS-P7-13 AC-2, ADR-016
func TestCapabilityRegistry_RegisterKnownDeploymentModels(t *testing.T) {
	for _, model := range []string{
		string(contracts.DeploymentModelServer),
		string(contracts.DeploymentModelHybrid),
		string(contracts.DeploymentModelPackage),
		"",
	} {
		t.Run("model_"+model, func(t *testing.T) {
			r := NewCapabilityRegistry()
			decl := laravelCapabilities()
			decl.DeploymentModel = model
			if err := r.Register("laravel", decl); err != nil {
				t.Fatalf("Register with deployment model %q failed: %v", model, err)
			}
			got, ok := r.Capabilities("laravel")
			if !ok || got.DeploymentModel != model {
				t.Errorf("Capabilities(\"laravel\").DeploymentModel = %q, want %q", got.DeploymentModel, model)
			}
		})
	}
}

// TestCapabilityRegistry_RegisterUnknownDeploymentModelRejected verifies
// that an unknown deployment model is rejected with a descriptive error
// naming the allowed values (TS-P7-13 AC-2).
//
// Reference: TS-P7-13 AC-2
func TestCapabilityRegistry_RegisterUnknownDeploymentModelRejected(t *testing.T) {
	r := NewCapabilityRegistry()
	decl := laravelCapabilities()
	decl.DeploymentModel = "cloud"

	err := r.Register("laravel", decl)
	if err == nil {
		t.Fatal("Register with unknown deployment model succeeded, want error")
	}
	for _, want := range []string{`unknown deployment model "cloud"`, `"server"`, `"hybrid"`, `"package"`} {
...
// TestCapabilityRegistry_RegisterInvalidBuildPhasesRejected verifies that
// empty and duplicate build phase names are rejected — the same rules as
// activation phases (TS-P7-14).
//
// Reference: TS-P7-14
func TestCapabilityRegistry_RegisterInvalidBuildPhasesRejected(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(decl *contracts.CapabilityDeclaration)
	}{
		{
			name: "empty_build_phase",
			mutate: func(decl *contracts.CapabilityDeclaration) {
				decl.BuildPhases = []string{"composer", ""}
			},
		},
		{
			name: "whitespace_build_phase",
			mutate: func(decl *contracts.CapabilityDeclaration) {
				decl.BuildPhases = []string{"   "}
			},
		},
		{
			name: "duplicate_build_phase",
			mutate: func(decl *contracts.CapabilityDeclaration) {
				decl.BuildPhases = []string{"composer", "npm", "composer"}
			},
		},
	}
...
// TestCapabilityRegistry_RegisterValidBuildPhases verifies that a
// declaration with distinct, non-empty build phase names registers
// successfully and queries back intact (TS-P7-14).
//
// Reference: TS-P7-14
func TestCapabilityRegistry_RegisterValidBuildPhases(t *testing.T) {
	r := NewCapabilityRegistry()
	decl := laravelCapabilities()
	...
	if !reflect.DeepEqual(got.BuildPhases, decl.BuildPhases) {
		t.Errorf("BuildPhases = %v, want %v", got.BuildPhases, decl.BuildPhases)
	}
}
