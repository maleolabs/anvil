...
// InvokeManifestCommands requests the adapter executable's activation and
// rollback command strings for the artifact manifest by dispatching the
// manifest command (CommandManifest). The command carries no request
// payload — the values are framework-inherent, so the invocation passes
// no JSON argument (the Process Runner has no stdin channel). The adapter
// returns a ManifestCommandResult JSON document on stdout, which the
// coordinator parses and returns; the CLI wiring layer stores the values
// in artifact.PackageOptions at packaging time (TS-P7-15, TS-P7-16,
// 005-adapter-command-contract §10.10). The method contains no
// framework-specific values.
//
// Reference: TS-P7-15, TS-P7-16, 005-adapter-command-contract §10.10
func (c *Coordinator) InvokeManifestCommands(ctx context.Context, framework, executable string) (contracts.ManifestCommandResult, error) {
	if err := c.ready(); err != nil {
		return contracts.ManifestCommandResult{}, err
	}

	request, err := execution.NewExecutionRequest(
		executable,
		execution.WithArgs([]string{contracts.CommandManifest}),
		execution.WithTimeout(adapterTimeout),
	)
	if err != nil {
		return contracts.ManifestCommandResult{}, fmt.Errorf("cannot invoke manifest commands for framework %q: %w", framework, err)
	}

	result := c.runner.Execute(ctx, request)
	if result.Status != execution.StatusSuccess {
		return contracts.ManifestCommandResult{}, fmt.Errorf("cannot invoke manifest commands for framework %q: adapter process failed: %s", framework, processFailure(result))
	}

	var parsed contracts.ManifestCommandResult
	if err := json.Unmarshal([]byte(result.Stdout), &parsed); err != nil {
		return contracts.ManifestCommandResult{}, fmt.Errorf("cannot invoke manifest commands for framework %q: adapter returned invalid JSON result: %w (stdout=%q status=%s)", framework, err, result.Stdout, result.Status)
	}
	return parsed, nil
}
...
// InvokeTemplate requests the adapter executable's pipeline definitions
// by dispatching the template command (CommandTemplate) with a
// TemplateRequest payload naming the framework (ADR-020 §1). The adapter
// returns a TemplateResult JSON document on stdout — the pipeline
// definitions it owns — which the coordinator parses and returns; the
// caller validates the definitions through the pipeline loader before
// writing them to .anvil/pipelines/ (never write unvalidated adapter
// output). The request is passed as a single JSON argument; the
// adapter's JSON result on stdout is authoritative. The method contains
// no framework-specific values.
//
// Reference: TS-007-038, ADR-020 §1
func (c *Coordinator) InvokeTemplate(ctx context.Context, framework, executable string, req contracts.TemplateRequest) (contracts.TemplateResult, error) {
	if err := c.ready(); err != nil {
		return contracts.TemplateResult{}, err
	}

	payload, err := json.Marshal(req)
	if err != nil {
		return contracts.TemplateResult{}, fmt.Errorf("cannot invoke template for framework %q: marshal request: %w", framework, err)
	}

	request, err := execution.NewExecutionRequest(
		executable,
		execution.WithArgs([]string{contracts.CommandTemplate, string(payload)}),
		execution.WithTimeout(adapterTimeout),
	)
	if err != nil {
		return contracts.TemplateResult{}, fmt.Errorf("cannot invoke template for framework %q: %w", framework, err)
	}

	result := c.runner.Execute(ctx, request)
	if result.Status != execution.StatusSuccess {
		return contracts.TemplateResult{}, fmt.Errorf("cannot invoke template for framework %q: adapter process failed: %s", framework, processFailure(result))
	}

	var parsed contracts.TemplateResult
	if err := json.Unmarshal([]byte(result.Stdout), &parsed); err != nil {
		return contracts.TemplateResult{}, fmt.Errorf("cannot invoke template for framework %q: adapter returned invalid JSON result: %w (stdout=%q status=%s)", framework, err, result.Stdout, result.Status)
	}
	return parsed, nil
}
