...
// against the REAL compiled Flutter standard executable (TS-P7-20,
// TS-016-02-01): the Flutter delivery lifecycle standard registers with
// the Core registries through the same path a project selecting the
// "flutter" adapter uses — capabilities and extension commands dispatched
...
//
// Since the repository split (TS-016-02-01, ADR-025), the Flutter
// standard executable is no longer built from this repository: it is
// built from the anvil-standard-flutter repository. The test compiles
// `cmd/flutter-adapter` there with `go build` and invokes the resulting
// binary as a subprocess — the same pattern as the pre-split test, and
// the same pattern the Laravel standard's binary test uses in the
// anvil-standard-laravel repository. The standard repository location is
// taken from ANVIL_STANDARD_FLUTTER_DIR (the local clone path the e2e
// suite uses, E2E_STANDARD_FLUTTER_DIR); the test skips when the
...
// package tests.
//
// Reference: TS-P7-20 AC-4, TS-P7-12, TS-016-02-01, ADR-009 §8.1, ADR-025
package adapter
...
// buildFlutterAdapterBinary compiles the Flutter standard executable
// from the anvil-standard-flutter repository into a temp dir and returns
// its path. The standard repository path comes from
// ANVIL_STANDARD_FLUTTER_DIR; the binary name mirrors the convention
// `anvil-adapter-<framework>` (005-adapter-command-contract §10).
func buildFlutterAdapterBinary(t *testing.T) string {
	...
	if _, err := os.Stat(filepath.Join(standardDir, "go.mod")); err != nil {
		t.Skipf("ANVIL_STANDARD_FLUTTER_DIR %q does not contain the anvil-standard-flutter module", standardDir)
	}
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("build flutter standard executable: %v\n%s", err, out)
	}
	return bin
}

// TestRegisterAdapterExecutable_FlutterAdapter verifies that the real
// Flutter standard executable registers with the Core registries: the
