...
//     for Laravel.
//
// "flutter" is a known roadmap framework whose template is not available yet
// (TS-P7-27), so it returns a "not yet supported" error rather than an
// "unknown framework" error. Any other value is unknown and rejected.
func NewFrameworkProjectConfig(name, framework string) (ProjectConfig, error) {
	...
	case "laravel":
		...
		return cfg, nil
	case "flutter":
		return ProjectConfig{}, fmt.Errorf("framework %q is not yet supported (template not available)", framework)
	default:
