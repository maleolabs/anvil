...
// sets Project.Framework and adjusts the artifact filter: Artifact.Include
// stays empty (a non-empty include list is a strict whitelist in the artifact
// filter and would drop every non-vendor file), and Artifact.Exclude carries
// the compiled default excludes minus "vendor/**" — vendor/ stays packaged
// because it is runtime-critical for Laravel (ADR-017) (TS-P7-29 AC-4).
func TestNewFrameworkProjectConfig_Laravel(t *testing.T) {
	...
	if len(cfg.Artifact.Include) != 0 {
		t.Errorf("Include = %v, want empty (include acts as a whitelist)", cfg.Artifact.Include)
	}

	// Exclude = compiled default excludes minus "vendor/**".
	if slices.Contains(cfg.Artifact.Exclude, "vendor/**") {
		t.Errorf("Exclude = %v, must not contain vendor/**", cfg.Artifact.Exclude)
	}
	for _, want := range []string{".git/**", "node_modules/**"} {
		if !slices.Contains(cfg.Artifact.Exclude, want) {
			t.Errorf("Exclude = %v, want it to contain %q", cfg.Artifact.Exclude, want)
		}
	}

	schemaEntry, ok := GetSchema().Entries["artifact.exclude"]
	if !ok {
		t.Fatal("GetSchema() has no artifact.exclude entry")
	}
	defaults, ok := schemaEntry.Default.([]string)
	if !ok {
		t.Fatalf("artifact.exclude default type = %T, want []string", schemaEntry.Default)
	}
	if len(cfg.Artifact.Exclude) != len(defaults)-1 {
		t.Errorf("Exclude has %d patterns, want %d (compiled defaults minus vendor/**)",
			len(cfg.Artifact.Exclude), len(defaults)-1)
	}
	// Ordering must follow the schema definition order (deterministic) and
	...
	if !slices.Equal(cfg.Artifact.Exclude, want) {
		t.Errorf("Exclude = %v, want %v (compiled defaults minus vendor/**)", cfg.Artifact.Exclude, want)
	}
}
...
// TestNewFrameworkProjectConfig_Flutter verifies that the "flutter" framework
// sets Project.Framework and leaves the artifact include/exclude lists at
// their compiled defaults (TS-P7-27 AC-1): Flutter build outputs are produced
// by the pipeline, so no artifact include override is needed.
func TestNewFrameworkProjectConfig_Flutter(t *testing.T) {
	cfg, err := NewFrameworkProjectConfig("app", "flutter")
	if err != nil {
		t.Fatalf("NewFrameworkProjectConfig() returned unexpected error: %v", err)
	}
	...
	if cfg.Project.Framework != "flutter" {
		t.Errorf("Framework = %q, want %q", cfg.Project.Framework, "flutter")
	}
	if len(cfg.Artifact.Include) != 0 {
		t.Errorf("Include = %v, want empty", cfg.Artifact.Include)
	}
