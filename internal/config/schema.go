...
func CoreSchema() Schema {
	return Schema{
		...
		Entries: map[string]SchemaEntry{
			...
			// --- Installer Customization (STO:installer-one-command-custom) ---
			"installer.name": {
				Key:         "installer.name",
				Type:        TypeString,
				Required:    false,
				Description: "Custom installer display name and bundle filename base (fallback to project.name).",
				Scope:       ScopeProject,
			},
			"installer.icon": {
				Key:         "installer.icon",
				Type:        TypeString,
				Required:    false,
				Description: "Path to launcher icon (png/ico) relative to project root, copied to Tauri bundle.",
				Scope:       ScopeProject,
			},
			...
			"installer.setup.env_map": {
				Key:         "installer.setup.env_map",
				Type:        TypeObject,
				Required:    false,
				Description: "Env map for setup binding: map of ENV_KEY -> template {{forms.<form>.<field>}} (e.g. DB_HOST: {{forms.database.db_host}}). For Laravel env write.",
				Scope:       ScopeProject,
			},
			"installer.setup.env_file": {
				Key:         "installer.setup.env_file",
				Type:        TypeString,
				Default:     ".env",
				Required:    false,
				Description: "Target env file path relative to install root for env_map write (default .env).",
				Scope:       ScopeProject,
			},
		},
	}
