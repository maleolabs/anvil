...
func Validate(schema Schema, config map[string]interface{}) []ValidationError {
	...
	// Installer forms schema validation (STO:installer-pipeline-core v3)
	// Parse installer.forms from flat map (handles both object and fragmented keys)
	if forms, err := ParseInstallerFormsFromFlat(config); err != nil {
		errs = append(errs, ValidationError{
			Key:      "installer.forms",
			Expected: "valid forms object",
			Actual:   err.Error(),
		})
	} else if forms != nil {
		formErrs := ValidateFormsSchema(forms)
