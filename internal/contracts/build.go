...
// Core invokes a standard's build pipeline — the build phases the
// standard declares in its capability declaration — through the `build`
// command (contracts.CommandBuild), and the standard reports the outcome
// of each phase through the build contract. The Core-side dispatch
// mechanism lives in internal/adapter; the Laravel build pipeline lives
// in the anvil-standard-laravel repository (TS-016-01-01, ADR-025 §6.2).
//
