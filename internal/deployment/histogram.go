// Package deployment — histogram for transport latency (p95 <30s AC1, reuse spike pattern).
//
// Reference: spikes/local-deploy-ssh-transport/histogram.go, sto:local-deploy-transport AC1
package deployment
...
// LatencySample is one push observation (AC1).
...
func (h *Histogram) Add(s LatencySample) { h.Samples = append(h.Samples, s) }
...
// P95 returns 95th percentile (AC1).
