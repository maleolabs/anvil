...
// TestReadSSHCredentialsFromEnv_KnownHosts verifies that the optional
// host key verification variables are read when set (TD-004).
func TestReadSSHCredentialsFromEnv_KnownHosts(t *testing.T) {
	setEnv(t, EnvDeployServerHost, "10.0.0.5")
	setEnv(t, EnvDeployServerUser, "deploy")
	setEnv(t, EnvDeploySSHKey, "/keys/id_ed25519")
	setEnv(t, EnvDeploySSHKnownHosts, "/etc/anvil/known_hosts")
	setEnv(t, EnvDeploySSHKnownHostsMode, "accept-new")

	creds, err := ReadSSHCredentialsFromEnv()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if creds.KnownHostsPath != "/etc/anvil/known_hosts" {
		t.Errorf("KnownHostsPath = %q, want %q", creds.KnownHostsPath, "/etc/anvil/known_hosts")
	}
	if creds.KnownHostsMode != KnownHostsModeAcceptNew {
		t.Errorf("KnownHostsMode = %q, want %q", creds.KnownHostsMode, KnownHostsModeAcceptNew)
	}
}

// TestReadSSHCredentialsFromEnv_KnownHostsUnset verifies that the
// optional host key verification variables default to "disabled" (empty
// path, strict mode) when unset — the legacy behavior (TD-004).
func TestReadSSHCredentialsFromEnv_KnownHostsUnset(t *testing.T) {
	setEnv(t, EnvDeployServerHost, "10.0.0.5")
	setEnv(t, EnvDeployServerUser, "deploy")
	setEnv(t, EnvDeploySSHKey, "/keys/id_ed25519")
	unsetEnv(t, EnvDeploySSHKnownHosts)
	unsetEnv(t, EnvDeploySSHKnownHostsMode)
	...
	if creds.KnownHostsPath != "" {
		t.Errorf("KnownHostsPath = %q, want empty (verification disabled)", creds.KnownHostsPath)
	}
	if creds.KnownHostsMode != KnownHostsModeStrict {
		t.Errorf("KnownHostsMode = %q, want default %q", creds.KnownHostsMode, KnownHostsModeStrict)
	}
}

// TestReadSSHCredentialsFromEnv_InvalidKnownHostsMode verifies that an
// unsupported DEPLOY_SSH_KNOWN_HOSTS_MODE produces a clear error naming
// the variable (TD-004).
func TestReadSSHCredentialsFromEnv_InvalidKnownHostsMode(t *testing.T) {
	setEnv(t, EnvDeployServerHost, "10.0.0.5")
	setEnv(t, EnvDeployServerUser, "deploy")
	setEnv(t, EnvDeploySSHKey, "/keys/id_ed25519")
	setEnv(t, EnvDeploySSHKnownHosts, "/etc/anvil/known_hosts")

	for _, raw := range []string{"yes", "no", "STRICT"} {
		setEnv(t, EnvDeploySSHKnownHostsMode, raw)
		_, err := ReadSSHCredentialsFromEnv()
		if err == nil {
			t.Errorf("expected error for DEPLOY_SSH_KNOWN_HOSTS_MODE=%q, got nil", raw)
			continue
		}
		if !strings.Contains(err.Error(), EnvDeploySSHKnownHostsMode) {
			t.Errorf("error %q should name %s", err.Error(), EnvDeploySSHKnownHostsMode)
		}
	}
}

// TestReadSSHCredentialsFromEnv_ModeWithoutFile verifies that an
// explicitly configured mode without a known-hosts file is rejected:
// it would silently mean "no verification", hiding a misconfiguration
// (TD-004, fail closed on configuration errors).
func TestReadSSHCredentialsFromEnv_ModeWithoutFile(t *testing.T) {
	setEnv(t, EnvDeployServerHost, "10.0.0.5")
	setEnv(t, EnvDeployServerUser, "deploy")
	setEnv(t, EnvDeploySSHKey, "/keys/id_ed25519")
	unsetEnv(t, EnvDeploySSHKnownHosts)

	for _, raw := range []string{"strict", "accept-new"} {
		setEnv(t, EnvDeploySSHKnownHostsMode, raw)
		_, err := ReadSSHCredentialsFromEnv()
		if err == nil {
			t.Errorf("expected error for %s=%q without %s, got nil",
				EnvDeploySSHKnownHostsMode, raw, EnvDeploySSHKnownHosts)
			continue
		}
		if !strings.Contains(err.Error(), EnvDeploySSHKnownHosts) {
			t.Errorf("error %q should name %s", err.Error(), EnvDeploySSHKnownHosts)
		}
		if !strings.Contains(err.Error(), EnvDeploySSHKnownHostsMode) {
			t.Errorf("error %q should name %s", err.Error(), EnvDeploySSHKnownHostsMode)
		}
