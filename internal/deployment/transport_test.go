...
// TestTransportError_Guidance verifies that Guidance() returns
// actionable recovery guidance for every failure class (TS-011-004
// AC-1..AC-4, EPIC-011 §7.6).
func TestTransportError_Guidance(t *testing.T) {
	tests := []struct {
		name        string
	...
	}{
		{
			...
			wantSubstrs: []string{
				"server address", "SSH service", "reachable",
			},
		},
		{
		...
		},
		{
			...
			wantSubstrs: []string{
				"authorized", "permission", "upload directory",
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := &TransportError{TargetID: TargetID("node-1"), Kind: tt.kind, Reason: "test"}
			got := err.Guidance()
			if got == "" {
				t.Fatal("Guidance() = empty, want non-empty actionable guidance")
			}
			for _, substr := range tt.wantSubstrs {
				if !strings.Contains(strings.ToLower(got), strings.ToLower(substr)) {
					t.Errorf("Guidance() = %q, want it to mention %q", got, substr)
				}
			}
		})
	}
