...
// frameworkBuildTemplates maps supported framework names to their build
// pipeline template generators (TS-P7-28, TS-P7-27).
//
// Selection is data-driven (ADR-009 §9.6 spirit): adding a framework template
// is a data change, not an if/else chain, and the engine stays
// framework-agnostic in structure. Template data is Core-owned (TS-P7-19).
// Frameworks without a registered template fall back to
// execution.DefaultBuildPipeline; the framework value itself is validated
// earlier by config.NewFrameworkProjectConfig.
var frameworkBuildTemplates = map[string]func() execution.PipelineDefinition{
	"laravel": execution.LaravelBuildPipeline,
	"flutter": execution.FlutterBuildPipeline,
}

// KnownFrameworks returns the sorted list of framework names with a
// registered build pipeline template (currently "laravel" and "flutter").
// It is the single source of truth for the set of frameworks the Core
// knows about: the CLI adapter commands enumerate available adapters from
// this set (TS-007-031, TS-007-033) and the engine selects build
// templates from it. The framework list itself is also accepted by
// config.NewFrameworkProjectConfig.
//
// Reference: TS-P7-27, TS-P7-28, TS-007-031, TS-007-033
func KnownFrameworks() []string {
	names := make([]string, 0, len(frameworkBuildTemplates))
	for name := range frameworkBuildTemplates {
...
// GenerateFrameworkPipelineConfigs generates the default pipeline YAML
// files (.anvil/pipelines/build.yaml and .anvil/pipelines/ci.yaml) for
// the project rooted at root, using the build template registered for
// framework when one exists. Existing pipeline files are skipped
// (non-destructive behaviour, TS-P7-28 AC-1). It is the public entry
// point for callers that need to materialize pipeline templates after
// initialization — e.g. the "anvil adapter use" command (TS-007-033)
// when a project selects a framework whose build template is missing.
//
// Reference: TS-P7-28, TS-007-033
...
//
// When a build template is registered for the given framework, build.yaml is
// generated from that template (TS-P7-28 AC-1); otherwise the default build
// pipeline is used. ci.yaml is always the default CI pipeline.
func generatePipelineConfigs(s project.Structure, framework string) error {
	...
	if template, ok := frameworkBuildTemplates[framework]; ok {
		buildDef = template()
	}
	...
	files := []pipelineFile{
		{name: project.PipelineBuildFileName, def: buildDef},
