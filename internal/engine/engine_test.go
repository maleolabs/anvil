...
// dependencies/assets/optimize), stores the framework in anvil.yaml, keeps
// artifact.include empty (a non-empty include list is a strict whitelist in
// the artifact filter), and carries the compiled default exclude list minus
// "vendor/**" so vendor/ stays packaged (runtime-critical for Laravel,
// ADR-017), and keeps ci.yaml as the default CI pipeline (TS-P7-28 AC-1,
// TS-P7-29 AC-1/AC-4).
...
// metadata), stores the framework in anvil.yaml, and keeps ci.yaml as the
// default CI pipeline (TS-P7-27 AC-1..AC-3).
