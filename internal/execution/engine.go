...
// ParsePipeline parses and validates a pipeline definition from YAML
// bytes. It is the loader's parse step — Load reads a file and delegates
// to it, and the pipeline template generator validates adapter-owned
// template output through it before writing .anvil/pipelines/ files
// (ADR-020 §1: never write unvalidated adapter output).
//
