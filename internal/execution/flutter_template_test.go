...
func TestFlutterBuildPipeline_Tasks(t *testing.T) {
	...
	wantTasks := []struct {
		...
		{name: "flutter-web", command: "flutter", args: []string{"build", "web"}},
		{name: "flutter-apk", command: "flutter", args: []string{"build", "apk", "--release"}},
		{name: "flutter-ios", command: "flutter", args: []string{"build", "ios", "--release"}},
	}
