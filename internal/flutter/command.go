...
// `validate`, `build`, and `manifest`. The `verify` command runs the
// Flutter verification checks (TS-P7-25) and the `validate` command
// validates Flutter config extension values (TS-P7-26). The `activate`
// command is intentionally absent: the hybrid deployment model has no
// server activation (TS-P7-20 AC-5, EPIC-007 §7.3) — invoking it reports
// an unknown command (ErrUnknownCommand, exit 2). The `manifest` command
// exists for contract symmetry and returns an empty result: the hybrid
// model has no activation/rollback commands to store in the artifact
// manifest (TS-P7-15, TS-P7-16).
package flutter
