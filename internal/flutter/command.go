...
// `validate`, and `build`. The `verify` command runs the Flutter
// verification checks (TS-P7-25) and the `validate` command validates
// Flutter config extension values (TS-P7-26). The `activate` command is
// intentionally absent: the hybrid deployment model has no server
// activation (TS-P7-20 AC-5, EPIC-007 §7.3) — invoking it reports an
// unknown command (ErrUnknownCommand, exit 2).
package flutter
