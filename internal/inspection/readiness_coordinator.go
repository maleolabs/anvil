...
//
// The serverRoot and configResolver parameters are passed through to the
// verification engine.
//
// Reference: TS-P9-02, ADR-003 §8.5
func (rc *ReadinessCoordinator) CheckReadiness(serverRoot string, configResolver interface{}) ReadinessCoordinatorResult {
	verificationResult := rc.engine.Verify(serverRoot, configResolver)
