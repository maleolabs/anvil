...
// TestReleaseInspector_Inspect_NoSharedLinksCheck verifies that the
// shared-links check is no longer part of release inspection: the
// shared-links model was demoted per ADR-031 §3 and the check that
// consumed it was removed (TS-015-04-02).
//
// Reference: TS-P9-07, ADR-031
func TestReleaseInspector_Inspect_NoSharedLinksCheck(t *testing.T) {
	dir := t.TempDir()
	cfg := runtime.DefaultRuntimeConfig()
	cfg.InstallRoot = dir
	...
	// The demoted shared-links model must not surface a check anymore.
	checkNames := make(map[string]bool)
	for _, c := range result.Checks {
		checkNames[c.Name] = true
	}
	if checkNames["shared_links"] {
		t.Error("Inspect() must not include the demoted shared_links check (ADR-031 §3, TS-015-04-02)")
	}
}
