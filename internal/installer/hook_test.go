...
func TestResolveEnvMap_ObjectAndFragmentedKeys(t *testing.T) {
	forms := map[string]map[string]string{
		"database": {
			"db_host":     "127.0.0.1",
			"db_name":     "forge",
	...
	cfg := map[string]interface{}{
		"installer.setup.env_map": map[string]interface{}{
			"DB_HOST":     "{{forms.database.db_host}}",
			"DB_DATABASE": "{{forms.database.db_name}}",
		},
		"installer.setup.env_map.DB_PASSWORD": "{{forms.database.db_password}}",
		"installer.setup.env_map.ADMIN_EMAIL": "{{forms.superAdmin.email}}",
	}

	got := ResolveEnvMap(cfg, forms)
	if got["DB_HOST"] != "127.0.0.1" {
		t.Fatalf("DB_HOST got %q", got["DB_HOST"])
	}
	if got["DB_DATABASE"] != "forge" {
		t.Fatalf("DB_DATABASE got %q", got["DB_DATABASE"])
	}
	if got["DB_PASSWORD"] != "secret-db-pass" {
		t.Fatalf("DB_PASSWORD got %q", got["DB_PASSWORD"])
	}
	if got["ADMIN_EMAIL"] != "owner@example.com" {
		t.Fatalf("ADMIN_EMAIL got %q", got["ADMIN_EMAIL"])
	}
}

func TestApplyEnvMap_WritesLaravelDotEnv(t *testing.T) {
	root := t.TempDir()
	...
	cfg := map[string]interface{}{
		"installer.setup.env_file": ".env",
		"installer.setup.env_map": map[string]interface{}{
			"DB_HOST":     "{{forms.database.db_host}}",
			"DB_DATABASE": "{{forms.database.db_name}}",
			"DB_USERNAME": "{{forms.database.db_username}}",
			"DB_PASSWORD": "{{forms.database.db_password}}",
		},
	}

	written, err := ApplyEnvMap(cfg, forms, root)
	if err != nil {
		t.Fatalf("ApplyEnvMap error: %v", err)
	}
	if written != envPath {
		t.Fatalf("written path got %q want %q", written, envPath)
	}
	...
	if strings.Contains(content, "DB_HOST=old-host") {
		t.Fatalf("old DB_HOST not replaced:\n%s", content)
	}
}
