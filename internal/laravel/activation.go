...
const (
	...
	// PhaseViewCache caches the application compiled views. It is not an
	// activation phase — view:cache is not reversible as a rollback
	// operation and is not part of the activation pipeline — but the
	// constant lives with the other artisan cache phases because the
	// build pipeline reuses it (TS-P7-14: the `view:cache` build phase).
