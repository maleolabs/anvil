...
func TestFormatDiagnosticView_Unhealthy(t *testing.T) {
	...
	// ADR-036 §3 (TS-015-05-02): recommendations-style output is demoted —
...
// the demoted view model (ADR-036 §3, TS-015-05-02) renders issues
...
func TestWriteDiagnosticJSON_EnvelopeAndData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// ADR-036 §3 (TS-015-05-02): the demoted view model never
			// emits recommendations or contexts fields.
			if _, ok := data["recommendations"]; ok {
				t.Errorf("data must not contain a recommendations field (ADR-036 demotion)")
			}
			if _, ok := data["contexts"]; ok {
				t.Errorf("data must not contain a contexts field (ADR-036 demotion)")
			}
		})
	}
}
