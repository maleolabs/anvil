...
func (b *DownloadBar) Finish() {
	...
	line := fmt.Sprintf("%s downloaded %s (%s)\n",
		b.s.Success(IconDone), b.s.Progress(b.name), formatBytes(b.tot))
	if !b.animated() {
...
func formatBytes(n int64) string {
	...
	if n < 1024*1024 { return fmt.Sprintf("%.1f KB", float64(n)/1024) }
	if n < 1024*1024*1024 { return fmt.Sprintf("%.1f MB", float64(n)/1024/1024) }
