...
//
// ── Error Presentation (TS-P8-06, Platform-010 §5.2, §7.4) ───────────────
//
...
//
// When writing to an interactive terminal, error output is additionally
// rendered in red (TS-008-009). Non-terminal writers (pipes, redirects,
// test buffers) always receive plain text; format functions stay pure.
//
// Reference: TS-P8-06, TS-008-009, Platform-010 §5.2, §7.4
package output
...
//
// Reference: TS-P8-06, TS-P8-07, Platform-010 §5.2, §8.1
type AppError struct {
	...
	//
	// Reference: TS-P8-07, Platform-010 §8.1
	ExitCodeValue int
}
...
//
// The rendered error is colored red when w is an interactive terminal
// and colors are enabled; plain text otherwise.
//
// Reference: TS-P8-06, TS-008-009, Platform-010 §5.2
func WriteAppError(w io.Writer, e *AppError) {
	if e == nil {
		return
	}
	fmt.Fprint(w, Red(w, FormatAppError(e)))
}
...
func WritePlainError(w io.Writer, message string) {
	fmt.Fprint(w, Red(w, FormatPlainError(message)))
}
