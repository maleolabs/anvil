...
// StepComplete overwrites the current step line with a check mark and
// duration.
func (r *InteractiveStepReporter) StepComplete(name string, duration time.Duration) {
	...
	connector := r.connector()
	checkmark := Green(r.w, "✓")
	fmt.Fprintf(r.w, "\033[1A\033[2K  %s %s %-40s %s\n", connector, checkmark, name, FormatDuration(duration))
	r.stepIdx++
}

// StepFailed overwrites the current step line with a cross mark and error.
