...
func (sp *StyledSpinner) animated() bool {
...
// away.
func NewStyledSpinner(s *Style, message string) *StyledSpinner {
	sp := &StyledSpinner{s: s, msg: message}
	if !sp.animated() {
...
// interfere.
func (sp *StyledSpinner) animate(stop chan struct{}) {
	defer sp.wg.Done()
	frame := 0
	draw := func() {
		fmt.Fprintf(sp.s.W, "\r\x1b[K%s%s %s",
