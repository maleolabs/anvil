...
// TestInteractiveStepReporter_TreeConnectors locks the tree rendering
// contract that the skill install/update pipelines depend on: every
// step line is indented two spaces, the first N-1 steps use the "├─"
// connector, and the LAST step (when the declared total matches the
// number of completed steps) uses "└─". Regression: the standard skill
// install/update paths declared SetTotal(7) for 8 steps, so the final
// "Record" step rendered a second "├─" tree after the "└─".
func TestInteractiveStepReporter_TreeConnectors(t *testing.T) {
	const steps = 8
	var buf bytes.Buffer
	r := NewInteractiveStepReporter(&buf)
	r.SetTotal(steps)
	for i := 0; i < steps; i++ {
		r.StepComplete(fmt.Sprintf("Step %d", i), time.Second)
	}

	lines := strings.Split(strings.TrimRight(buf.String(), "\n"), "\n")
	if len(lines) != steps {
		t.Fatalf("rendered %d lines, want %d: %q", len(lines), steps, lines)
	}
	for i, line := range lines {
		if !strings.HasPrefix(line, "  ") {
			t.Errorf("line %d must be indented two spaces: %q", i+1, line)
		}
		want := "├─"
		if i == steps-1 {
			want = "└─"
		...
		if !strings.Contains(line, want) {
			t.Errorf("line %d must use %q connector, got %q", i+1, want, line)
		}
	}
}
