...
func TestInteractiveStepReporter_Start(t *testing.T) {
	...
	if !strings.Contains(got, "▶") {
		t.Errorf("Start() should contain ▶, got %q", got)
	}
	if !strings.Contains(got, "Update Anvil CLI") {
		t.Errorf("Start() should contain title, got %q", got)
	}
...
func TestInteractiveStepReporter_StepStart(t *testing.T) {
	var buf bytes.Buffer
	r := NewInteractiveStepReporter(&buf)
	...
	got := buf.String()
	if !strings.Contains(got, "├─") {
		t.Errorf("StepStart() should contain tree connector, got %q", got)
	}
	if !strings.Contains(got, "Download") {
		t.Errorf("StepStart() should contain step name, got %q", got)
	}
}
