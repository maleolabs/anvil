...
// Pause halts the in-place TTY animation and moves the cursor to a fresh
// line so external command output (pipeline ci/build) does not interleave
// with the tree's \r redraws. Call before running an external tool;
// capture its stdout/stderr via Runner (which buffers), then render the
// captured output as indented blocks under the task line and resume with
// Render(). On non-TTY Pause is a no-op (plain sequential lines never use
// \r).
func (t *Tree) Pause() {
	if t.s.TTY && t.s.Color && t.nodeLines > 0 {
		fmt.Fprintln(t.s.Raw())
		t.nodeLines = 0
	}
}

// Resume redraws the tree after a Pause. Convenience wrapper for Render().
func (t *Tree) Resume() { t.Render() }
...
// Last node uses TreeLast (└──) and detail uses spaces, others use TreeBranch/TreeVert.
func (t *Tree) renderPlain() {
	for idx, n := range t.nodes {
		...
		if isLast {
			prefix = TreeSpace + TreeSpace
		}
		if n.state == stateFailed {
			fmt.Fprintf(t.s.W, "%s%s\n", prefix, t.s.Error("failed: "+n.detail))
		} else {
			fmt.Fprintf(t.s.W, "%s%s %s\n", prefix, t.s.Success(IconDone), n.detail)
		}
	}
}
...
// output. The redraw writes bypass the margin writer (Raw) and embed
// the container margin itself after each erase — exactly like the
// spinner redraws: carriage-return-prefixed writes are skipped by the
// margin writer, so the in-place lines must carry their own margin.
func (t *Tree) renderTTY() {
	lines := t.ttyLines()
	w := t.s.Raw()
	if t.nodeLines > 0 {
		fmt.Fprintf(w, "\x1b[%dA", t.nodeLines)
	}
	for i, line := range lines {
		if i > 0 {
			fmt.Fprint(w, "\n")
		}
		fmt.Fprintf(w, "\r\x1b[K%s%s", Margin, line)
	}
...
// ttyLines renders the current node lines for the in-place redraw.
// Last node uses clean prefix without dangling │.
func (t *Tree) ttyLines() []string {
	...
	for idx, n := range t.nodes {
		...
		case stateDone:
			...
			lines = append(lines, branch+" "+t.s.Success(IconDone+" "+n.label))
			if n.detail != "" {
				prefix := TreeVert + TreeSpace
				if isLast {
					prefix = TreeSpace + TreeSpace
				}
				lines = append(lines, prefix+t.s.Success(IconDone+" "+n.detail))
			}
		case stateFailed:
			branch := TreeBranch
			if isLast {
			...
			lines = append(lines, branch+" "+t.s.Error(n.label))
			if n.detail != "" {
				prefix := TreeVert + TreeSpace
				if isLast {
					prefix = TreeSpace + TreeSpace
				}
				lines = append(lines, prefix+t.s.Error("failed: "+n.detail))
			}
		default:
			branch := TreeBranch
			if isLast {
			...
			lines = append(lines, branch+" "+t.s.Dim(n.label))
		}
	}
	return lines
}
