...
// carries the version it was written with, and reads accept exactly
// RecordFormatVersion. The validation-result section (Compatibility,
// Trust) is part of the pinned format; changing its shape is a format
// change — the migration path is bumping RecordFormatVersion and teaching
// reads to handle both versions, not silently tolerating unknown shapes.
...
type InstalledStandardRecord struct {
	...
	// written with; reads require exactly RecordFormatVersion.
...
func (s *InstalledStandardStore) Record(id string, rec InstalledStandardRecord) (InstalledStandardRecord, bool, error) {
	if err := validateRecord(id, rec); err != nil {
...
func (s *InstalledStandardStore) List() ([]InstalledStandardSummary, []CorruptRecord, error) {
	...
	var summaries []InstalledStandardSummary
...
// exactly RecordFormatVersion, and the core fields are present: pinned
...
func validateRecord(id string, rec InstalledStandardRecord) error {
	var problems []string
	if id == "" {
	...
	if rec.FormatVersion != RecordFormatVersion {
		problems = append(problems, fmt.Sprintf(
			"formatVersion must be %d, got %d — the record format version is pinned; records written in another format version must be migrated (bump RecordFormatVersion and teach reads to handle both versions)",
