...
func TestInstalledStandardStoreRecordRejectsInvalidRecords(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			want: "formatVersion must be 1",
		},
		{
			name: "unknown format version",
			id:   valid.ID,
			rec:  func() InstalledStandardRecord { r := valid; r.FormatVersion = 2; return r }(),
			want: "formatVersion must be 1",
		},
		{
...
// read: a file carrying any version other than RecordFormatVersion is
// corrupt — the record format is pinned (CR: format version + typed
// validation results; migration path = bump RecordFormatVersion).
func TestInstalledStandardStoreFormatVersionWrittenAndEnforced(t *testing.T) {
	...
	// Written: the persisted file carries formatVersion 1.
	var fileRec InstalledStandardRecord
	...
	// Enforced: a future-format file is rejected as corrupt, on Get and
	// in List, with an actionable message naming the format version.
