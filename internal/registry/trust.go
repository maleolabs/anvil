...
// VerifyAttestationAnchored verifies the publisher attestation and the
// out-of-band trust anchor match of one metadata document — the
// attestation+anchor half of VerifyTrust, without the content-integrity
// half (which needs the release content bytes). It is the trust gate the
// skill install path applies to the resolved metadata BEFORE any fetch
// (ADR-037 D4: trust anchors before fetch): the skill asset itself is
// then verified against the attestation-bound named digest
// (VerifyAssetDigest, TS-014-04-04).
//
// Fail-closed: Valid is false unless BOTH checks pass. anchors may be nil
// or an empty store — verification then always fails with the actionable
// no-anchor message (no first-use acceptance, no privileged path; PM
// decision D-07). The result carries the same auditable fields as
// VerifyTrust (publisher, declared signature/key, anchor path) but never
// performs content integrity checks, so DeclaredDigests and
// IntegrityVerified stay empty/false.
//
// Reference: TS-014-04-02, ADR-022 §3, ADR-037 D4
func VerifyAttestationAnchored(md Metadata, anchors *TrustAnchors) TrustResult {
	result := TrustResult{
		Publisher:         md.ID,
		DeclaredSignature: md.Trust.Attestation.Signature,
		DeclaredPublicKey: md.Trust.Attestation.PublicKey,
	...
	result.Valid = result.AttestationVerified && result.AnchorMatched
	return result
}
...
func VerifyAssetDigest(md Metadata, assetName, sha256Hex string) (bool, error) {
	...
	if !bytes.Equal(actual, expected) {
		// The recomputed (downloaded) digest is deliberately NOT included
		// in the message, mirroring the content-digest policy of
		// checkIntegrity: it would leak a hash oracle when the installer
		// is pointed at content it should not fetch. The declared digest
		// (attestation-bound, signed material) and the mismatch fact are
		// actionable; the recomputed value is not.
		return true, fmt.Errorf(
			"attestation-bound digest mismatch for asset %q: the downloaded bytes do not match the digest declared in the release's attested metadata (declared %s %s). The binary was tampered with or the release is broken — the install is aborted (ADR-022 §3; TS-014-04-04). Re-fetch the binary from the release channel %s, or report the broken release to the publisher.",
			assetName, declared.Encoding, declared.Digest, md.Distribution.Location)
	}
	return true, nil
}
