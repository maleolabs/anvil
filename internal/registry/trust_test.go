...
// TestVerifyAttestationAnchored asserts the attestation+anchor gate used
// by the skill install path BEFORE any fetch (ADR-037 D4): it runs the
// publisher-attestation and trust-anchor checks without needing content
// bytes, and is fail-closed — a wrong key, an unknown publisher, or a
// nil/empty anchor store all yield Valid=false.
func TestVerifyAttestationAnchored(t *testing.T) {
	content := testContent()
	...
	t.Run("valid-anchored", func(t *testing.T) {
		...
		result := VerifyAttestationAnchored(md, testAnchors(t, pub))

		if !result.Valid {
			t.Errorf("Valid = false, want true (errors: %v)", result.Errors)
		}
		if !result.AttestationVerified || !result.AnchorMatched {
			t.Errorf("Valid = true but checks not both verified: %+v", result)
		}
		if result.IntegrityVerified {
			t.Error("IntegrityVerified = true — the helper never runs content integrity (no content bytes)")
		}
	})

	t.Run("declared-key-not-anchored", func(t *testing.T) {
		// Attestation signed by key A, anchored key B: the signature
		// verifies with the declared key A, but origin fails — the
		// publisher is not trusted (no TOFU, PM decision D-07).
		signerPub, signerPriv := testEd25519Keypair(t)
		anchoredPub, _ := testEd25519Keypair(t)
		md := testRelease(t, content, signerPub, signerPriv)

		result := VerifyAttestationAnchored(md, testAnchors(t, anchoredPub))

		if result.Valid {
			t.Error("Valid = true for a non-anchored declared key, want false")
		}
		if !result.AttestationVerified {
			t.Error("AttestationVerified = false, want true — the signature does verify with the declared key")
		}
		if result.AnchorMatched {
			t.Error("AnchorMatched = true, want false — the declared key differs from the anchor")
		}
		if !hasMessage(result.Errors, "does not match the trusted anchor") {
			t.Errorf("Errors = %v, want an anchor-mismatch message", result.Errors)
		}
	})
	...
	t.Run("tampered-signature", func(t *testing.T) {
		...
		if result.AttestationVerified {
			t.Error("AttestationVerified = true for a tampered signature, want false")
		}
	})

	t.Run("no-anchors-fail-closed", func(t *testing.T) {
		pub, priv := testEd25519Keypair(t)
		md := testRelease(t, content, pub, priv)

		result := VerifyAttestationAnchored(md, nil)

		if result.Valid {
			t.Error("Valid = true with no anchors, want false (no TOFU)")
		}
		if !result.AttestationVerified {
			t.Error("AttestationVerified = false, want true — the signature verifies with the declared key")
		}
		if !hasMessage(result.Errors, "no trust anchor") {
	...
	t.Run("unknown-publisher", func(t *testing.T) {
		...
		// The anchor store only knows a different publisher — origin
		// fails even though the signature verifies (PM decision D-07).
		anchors, err := TrustAnchorsFromKeys(map[string]string{
			"anvil-standard-other": base64.StdEncoding.EncodeToString(pub),
		})
		if err != nil {
			t.Fatalf("build trust anchors: %v", err)
		}

		result := VerifyAttestationAnchored(md, anchors)
