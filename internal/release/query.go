...
//
// The query interface supports four operations:
//  1. GetStateByID  — returns the current lifecycle stage for a Release
//  2. ListAll       — returns every Release with its lifecycle stage
//  3. ListByStage   — returns all Releases in a given lifecycle stage
//  4. GetActive     — returns the Release currently in Active stage
//
...
// ListReleases returns every Release in the runtime's releases state
// directory regardless of lifecycle stage. Returns an empty slice if no
// Releases exist or the releases state directory does not exist yet.
//
// Entries that cannot be loaded (e.g. corrupt JSON) are skipped, matching
// the behavior of ListReleasesByStage.
//
// Reference: TS-P4-09, TS-012-002
func ListReleases(runtimePath string) ([]*Release, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("list releases: read releases dir: %w", err)
	}
...
func ListReleasesByStage(runtimePath string, stage Stage) ([]*Release, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list releases by stage: %w", err)
	}
