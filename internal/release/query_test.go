...
// TestListReleases_AllStages verifies that ListReleases returns every
// Release regardless of lifecycle stage.
//
// Reference: TS-012-002
func TestListReleases_AllStages(t *testing.T) {
	runtimePath := setupQueryTest(t)

	stages := []Stage{StageReady, StageActivating, StageActive, StageRolledBack, StageArchived, StageFailed}
	for i, stage := range stages {
		rel := &Release{
			ID:          ReleaseID(fmt.Sprintf("test-status-%03d", i)),
			Stage:       stage,
			Transitions: []TransitionRecord{},
		}
		saveTestRelease(t, runtimePath, rel)
	...
	all, err := ListReleases(runtimePath)
	if err != nil {
		t.Fatalf("ListReleases returned unexpected error: %v", err)
	}

	if len(all) != len(stages) {
		t.Fatalf("expected %d releases, got %d", len(stages), len(all))
	}

	// Every saved stage must be present in the listing.
	found := map[Stage]bool{}
	for _, rel := range all {
		found[rel.Stage] = true
	}
	for _, stage := range stages {
		if !found[stage] {
			t.Errorf("release with stage %s missing from listing", stage)
		}
	}
}

// TestListReleases_EmptyStateDir verifies that ListReleases returns an
// empty slice when the releases state directory does not exist.
//
// Reference: TS-012-002
func TestListReleases_EmptyStateDir(t *testing.T) {
	runtimePath := t.TempDir()

	result, err := ListReleases(runtimePath)
	if err != nil {
		t.Fatalf("ListReleases returned unexpected error: %v", err)
	}

	if result == nil {
		t.Fatal("ListReleases returned nil, expected empty slice")
	}
	if len(result) != 0 {
		t.Errorf("expected 0 releases, got %d", len(result))
	}
...
// TestListReleases_SkipsUnreadableFiles verifies that ListReleases skips
// entries that cannot be loaded (e.g. corrupt JSON) instead of failing.
//
// Reference: TS-012-002
func TestListReleases_SkipsUnreadableFiles(t *testing.T) {
	runtimePath := setupQueryTest(t)

	rel := &Release{
		ID:          ReleaseID("test-valid-001"),
		Stage:       StageReady,
		Transitions: []TransitionRecord{},
	}
	saveTestRelease(t, runtimePath, rel)
	...
	if len(result) != 1 {
		t.Errorf("expected 1 readable release, got %d", len(result))
	}
	if result[0].ID != rel.ID {
		t.Errorf("listed release ID = %s, want %s", result[0].ID, rel.ID)
	}
}
