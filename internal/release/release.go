...
// ErrReleaseNotFound is returned by Load (and lookups built on it) when
// no Release file exists for the given identity. It lets the command
// surface classify a not-found lookup as the runtime category (exit 3,
// "resource not found") instead of a general error (TS-019-03-02, F-02).
...
func Load(path string) (*Release, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("release file not found: %s: %w", path, ErrReleaseNotFound)
		}
