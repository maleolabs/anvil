...
func (e *RollbackEngine) Rollback() (*RollbackResult, error) {
	...
	// ----------------------------------------------------------------
	// Phase 2b: Re-verify archived artifact before promotion (AC4, HIGH gap close).
	// The archived artifact is the target being restored to Active — its
	// stored bytes must be re-proven intact BEFORE any stage transition or
	// symlink switch, mirroring the Activate verification-before-trust gate.
	// A tampered or corrupt archived artifact (TOCTOU window between archiving
	// and rollback) must be rejected with no promotion and no state mutation
	// — the active release stays Active. When the release carries no artifact
	// path or the file does not exist (pre-patch fixtures / synthetic paths
	// like "/tmp/artifacts/target.tar.gz"), the check is skipped and the
	// stage transition remains the authority (BUG-002 legacy compat).
	// ----------------------------------------------------------------
	if target.ArtifactPath != "" {
		if _, err := os.Stat(target.ArtifactPath); err == nil {
			if err := artifact.RequireVerified(target.ArtifactPath); err != nil {
				return nil, fmt.Errorf("rollback failed at Verification phase: archived artifact %s verification failed: %w — refusing rollback (verification-before-trust)", target.ArtifactPath, err)
			}
		} else if !os.IsNotExist(err) {
			return nil, fmt.Errorf("rollback failed at Verification phase: access archived artifact %s: %w", target.ArtifactPath, err)
		}
	}
...
//
// The identification logic is the shared read-only query FindRollbackTarget
// (observability.go) — the rollback engine and the observability surface
// derive the target from the same authoritative lifecycle state.
func (e *RollbackEngine) findRollbackTarget() (*Release, error) {
	return FindRollbackTarget(e.runtimePath)
}
