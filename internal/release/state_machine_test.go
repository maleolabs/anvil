...
// TestStateMachine_Save_CrashWindowAtomic verifies the TD-002 crash-window
// property for StateMachine.Save: a crash mid-save (simulated by a partial
// temp file that never got renamed) leaves the complete previous state
// machine file at the final path, so Load never observes a truncated state.
// A subsequent Save recovers and leaves no temp files behind.
//
// Reference: TD-002, TS-P4-04
func TestStateMachine_Save_CrashWindowAtomic(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state_machine.json")

	// Persist the previous complete state machine state.
	sm := NewStateMachine(StageReady)
	if err := sm.Transition(StageActivating); err != nil {
		t.Fatalf("Transition(Activating) unexpected error: %v", err)
	}
	...
	crashTemp := filepath.Join(dir, "state_machine.json.tmp-crashed")
	if err := os.WriteFile(crashTemp, []byte(`{"stage":1,"transitions":[{"t`), 0644); err != nil {
	...
	if got := loaded.Stage(); got != StageActivating {
		t.Errorf("stage after simulated crash = %s, want %s (previous complete state)", got, StageActivating)
	}

	// A subsequent Save must succeed and persist the new complete state.
	sm2 := NewStateMachine(StageReady)
	if err := sm2.Transition(StageActivating); err != nil {
		t.Fatalf("Transition(Activating) unexpected error: %v", err)
	}
	if err := sm2.Transition(StageActive); err != nil {
		t.Fatalf("Transition(Active) unexpected error: %v", err)
	}
	if err := sm2.Save(path); err != nil {
		t.Fatalf("Save() after crash returned unexpected error: %v", err)
	}

	loaded2 := NewStateMachine(StageReady)
	if err := loaded2.Load(path); err != nil {
		t.Fatalf("Load() after recovery returned unexpected error: %v", err)
	}
	if got := loaded2.Stage(); got != StageActive {
		t.Errorf("stage after recovery = %s, want %s", got, StageActive)
	}
}

// TestStateMachine_Save_ReplacesCorruptFile verifies that StateMachine.Save
// atomically replaces a corrupt state machine file at the final path (the
// artifact of the pre-TD-002 non-atomic writer) with a complete, loadable
// state.
//
// Reference: TD-002
func TestStateMachine_Save_ReplacesCorruptFile(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state_machine.json")

	if err := os.WriteFile(path, []byte(`{"stage":1,"transitions":[{"`), 0644); err != nil {
		t.Fatalf("failed to write corrupt state machine file: %v", err)
	}
	...
	if got := loaded.Stage(); got != StageActivating {
		t.Errorf("stage = %s, want %s", got, StageActivating)
	}
}
