...
// OperationLockRecord describes a lifecycle operation currently holding
// the server operation lock (TS-015-04-03). It is the persisted half of
// the lock state: the flock on runtime-state.lock is the authority, this
// record is the observation — diagnostics and status queries can report
// which operation is in flight (ADR-014: Runtime State contains locks;
// ADR-036 §3: observability observes, it never gates).
type OperationLockRecord struct {
	// Operation is the lifecycle operation holding the lock
	// (install, activate, rollback).
	Operation string `json:"operation"`
	// PID is the process holding the lock.
	PID int `json:"pid"`
	// AcquiredAt is when the lock was acquired (UTC).
...
type RuntimeState struct {
	...
	OperationLock        *OperationLockRecord `json:"operation_lock,omitempty"`
...
// SetOperationLock records the lifecycle operation holding the server
// operation lock (TS-015-04-03). The record is persisted by Save so lock
// state is part of lifecycle state handling (ADR-014: Runtime State
// contains locks); it is cleared by ClearOperationLock when the operation
// finishes.
func (s *StateStore) SetOperationLock(operation string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.state.OperationLock = &OperationLockRecord{
		Operation:  operation,
		PID:        os.Getpid(),
...
// ClearOperationLock removes the operation lock record.
func (s *StateStore) ClearOperationLock() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.state.OperationLock = nil
	s.state.LastUpdated = time.Now()
}
...
// a crash mid-save leaves either the complete previous state or the complete
// new state at the path — never a truncated or partially-written state file
// (TD-002). RuntimeState is the operational record that observability and
// rollback read after a restart, so it must always be parseable.
//
// Reference: TS-P5-04 AC-1, TD-002
func (s *StateStore) Save() error {
	...
	sf := stateFile{
		...
		OperationLock:        s.state.OperationLock,
	...
	if err := fsutil.WriteFileAtomic(s.path, data, 0644); err != nil {
...
func (s *StateStore) Load() error {
	...
	s.state = RuntimeState{
		...
		OperationLock:        sf.OperationLock,
