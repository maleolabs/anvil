...
func registerE2EProject(t *testing.T, serverRoot string) (projectID, installRoot string) {
	...
	// Shared env gate: Activate requires valid shared/.env.
...
// content). After the verification-before-trust patch (sto:local-deploy-
// coordinator, HIGH gap close), corruption is rejected at the verification
// gate BEFORE extraction, so the error mentions verification failure.
func TestActivate_ExtractFailure(t *testing.T) {
	...
	// Verification-before-trust rejects before extraction (AC1), so accept
	// either the verification or extraction error wording.
	if !contains(err.Error(), "extract artifact for activation") && !contains(err.Error(), "stored artifact verification failed") && !contains(err.Error(), "verification-before-trust") {
		t.Errorf("expected error to mention 'extract artifact for activation' or 'stored artifact verification failed', got: %v", err)
	}
}
