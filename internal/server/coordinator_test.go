...
func setupServerEnv(t *testing.T, serverRoot string) (projectID, installRoot string) {
	...
	// Shared env required for Activate (console build-once gate).
	if err := os.MkdirAll(filepath.Join(installRoot, "shared"), 0755); err != nil {
		t.Fatalf("mkdir shared env dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(installRoot, "shared", ".env"), []byte("APP_ENV=production\nAPP_KEY=base64:testkey1234567890\n"), 0644); err != nil {
...
func setupActivateRelease(t *testing.T, projectID, installRoot, releaseID string) string {
	...
	// Ensure shared env gate passes (console build-once flow).
	// Coordinator Activate now checks <installRoot>/shared/.env exists and
	// has valid syntax before promotion; without it the test fixtures would
	// fail with "run database provision". Create a minimal valid .env.
	sharedEnvDir := filepath.Join(installRoot, "shared")
	if err := os.MkdirAll(sharedEnvDir, 0755); err != nil {
		t.Fatalf("mkdir shared env dir: %v", err)
	}
	sharedEnvPath := filepath.Join(sharedEnvDir, ".env")
...
// ---------------------------------------------------------------------------
// Shared env verify gate (console build-once integration)
// ---------------------------------------------------------------------------

// TestActivate_MissingSharedEnvFailsWithProvisionHint verifies the
// pre-promote gate (console integration audit item 1): activating a Ready
// release when shared/.env is missing fails with an actionable hint
// "run database provision" instead of promoting a broken release.
func TestActivate_MissingSharedEnvFailsWithProvisionHint(t *testing.T) {
	serverRoot := t.TempDir()
	releaseID := "rel-missing-env"
	projectID, releasePath := setupActivateEnvironment(t, serverRoot, releaseID)

	// Remove the shared env that setupActivateEnvironment created.
	reg, _ := NewRegistryStore(serverRoot).Load(projectID)
	sharedEnv := filepath.Join(reg.Project.InstallRoot, "shared", ".env")
	if err := os.Remove(sharedEnv); err != nil {
		t.Fatalf("remove shared env: %v", err)
	}

	coord := NewServerReleaseCoordinator(serverRoot)
	err := coord.Activate(projectID, releaseID)
	if err == nil {
		t.Fatal("expected error for missing shared env, got nil")
	}
	if !contains(err.Error(), "run database provision") {
		t.Errorf("error must hint 'run database provision', got: %v", err)
	}
	if !contains(err.Error(), "shared env not found") {
		t.Errorf("error must mention shared env not found, got: %v", err)
	}
	// Release must not have transitioned to Active.
	rel, _ := release.Load(releasePath)
	if rel.Stage == release.StageActive {
		t.Error("release must not be Active when shared env gate fails")
	}
}

// TestActivate_EmptySharedEnvFailsWithProvisionHint verifies empty file handling.
func TestActivate_EmptySharedEnvFailsWithProvisionHint(t *testing.T) {
	serverRoot := t.TempDir()
	releaseID := "rel-empty-env"
	projectID, _ := setupActivateEnvironment(t, serverRoot, releaseID)
	reg, _ := NewRegistryStore(serverRoot).Load(projectID)
	sharedEnv := filepath.Join(reg.Project.InstallRoot, "shared", ".env")
	if err := os.WriteFile(sharedEnv, []byte(""), 0644); err != nil {
		t.Fatalf("truncate shared env: %v", err)
	}
	coord := NewServerReleaseCoordinator(serverRoot)
	err := coord.Activate(projectID, releaseID)
	if err == nil {
		t.Fatal("expected error for empty shared env, got nil")
	}
	if !contains(err.Error(), "run database provision") {
		t.Errorf("error must hint 'run database provision', got: %v", err)
	}
...
// TestActivate_InvalidSharedEnvSyntaxFails verifies syntax validation.
func TestActivate_InvalidSharedEnvSyntaxFails(t *testing.T) {
	serverRoot := t.TempDir()
	releaseID := "rel-bad-env"
	projectID, _ := setupActivateEnvironment(t, serverRoot, releaseID)
	reg, _ := NewRegistryStore(serverRoot).Load(projectID)
	sharedEnv := filepath.Join(reg.Project.InstallRoot, "shared", ".env")
	// Missing '=' on line 2.
	...
	err := coord.Activate(projectID, releaseID)
	if err == nil {
		t.Fatal("expected error for invalid shared env syntax, got nil")
	}
	if !contains(err.Error(), "run database provision") {
		t.Errorf("error must hint 'run database provision', got: %v", err)
	}
	if !contains(err.Error(), "invalid syntax") && !contains(err.Error(), "syntax error") {
		t.Errorf("error must mention syntax, got: %v", err)
	}
}

// TestActivate_InvalidSharedEnvKeyFails verifies key charset validation.
func TestActivate_InvalidSharedEnvKeyFails(t *testing.T) {
	serverRoot := t.TempDir()
	releaseID := "rel-badkey-env"
	projectID, _ := setupActivateEnvironment(t, serverRoot, releaseID)
	reg, _ := NewRegistryStore(serverRoot).Load(projectID)
	sharedEnv := filepath.Join(reg.Project.InstallRoot, "shared", ".env")
	if err := os.WriteFile(sharedEnv, []byte("good_key=value\n"), 0644); err != nil {
		t.Fatalf("write bad key env: %v", err)
	}
	coord := NewServerReleaseCoordinator(serverRoot)
	err := coord.Activate(projectID, releaseID)
	if err == nil {
...
// TestActivate_ValidSharedEnvPasses verifies the happy path with comments and blanks.
func TestActivate_ValidSharedEnvPasses(t *testing.T) {
	serverRoot := t.TempDir()
	releaseID := "rel-valid-env"
	projectID, releasePath := setupActivateEnvironment(t, serverRoot, releaseID)
	reg, _ := NewRegistryStore(serverRoot).Load(projectID)
	sharedEnv := filepath.Join(reg.Project.InstallRoot, "shared", ".env")
	if err := os.WriteFile(sharedEnv, []byte("# comment\n\nAPP_ENV=production\nAPP_KEY=secret\n# end\n"), 0644); err != nil {
		t.Fatalf("write valid env: %v", err)
	}
	coord := NewServerReleaseCoordinator(serverRoot)
	if err := coord.Activate(projectID, releaseID); err != nil {
		t.Fatalf("Activate with valid env returned unexpected error: %v", err)
	}
	rel, _ := release.Load(releasePath)
	if rel.Stage != release.StageActive {
		t.Errorf("Stage = %s, want %s", rel.Stage, release.StageActive)
	}
}

// TestValidateEnvContent isolates the syntax helper (mirrors console Validations).
func TestValidateEnvContent(t *testing.T) {
	cases := []struct {
		content string
		wantErr bool
	}{
	...
	for i, tc := range cases {
		err := validateEnvContent(tc.content)
		if (err != nil) != tc.wantErr {
			t.Errorf("case %d validateEnvContent(%q) err=%v wantErr=%v", i, tc.content, err, tc.wantErr)
		}
	}
}
