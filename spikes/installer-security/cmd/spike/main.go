...
func main() {
	...
	if outDir == "" { outDir = filepath.Join(repoRoot, "dist", "installer") }
	if evidenceDir == "" { evidenceDir = filepath.Join(os.TempDir(), "spike-installer-security-evidence") }
	_ = os.MkdirAll(evidenceDir, 0755)
	_ = os.MkdirAll(outDir, 0755)

	harnessLogPath := filepath.Join(evidenceDir, "harness.log")
	f, err := os.Create(harnessLogPath)
	if err != nil { fmt.Fprintf(os.Stderr, "create harness.log: %v\n", err); os.Exit(1) }
	defer f.Close()
