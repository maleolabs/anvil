...
func RunSecurityHarness(cfg HarnessConfig) (*HarnessResult, error) {
	...
	start := time.Now()
	if err := os.MkdirAll(cfg.EvidenceDir, 0755); cfg.EvidenceDir != "" && err != nil {
		return nil, err
	}
	if err := os.MkdirAll(cfg.OutputDir, 0755); cfg.OutputDir != "" && err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("package trusted artifact: %w", err)
	}
	...
	if err := TamperArtifact(trustedArtifact, tamperedPath); err != nil {
		return nil, err
	}
	...
	if vrTrusted != nil && vrTrusted.Passed {
		...
		if errOk == nil {
			fmt.Fprintln(mw, "[AC1] trusted artifact PASS (no false positive)")
		}
	}
	...
	if b, _ := json.MarshalIndent(binding, "", "  "); b != nil {
		_ = os.WriteFile(bindingPath, b, 0644)
	}
	...
	if err != nil {
		payloadLog.WriteString(fmt.Sprintf("VerifyInstallerPayloadIntegrity error: %v\n", RedactInstallerLog(err.Error())))
	}
	if pass {
	...
	if !res.AC2_PayloadIntegrity || !res.AC2_RepackDetected {
		return nil, fmt.Errorf("AC2 FAIL: integrity=%t repack=%t", res.AC2_PayloadIntegrity, res.AC2_RepackDetected)
	}
	...
	if !res.AC3_Redacted {
		return nil, fmt.Errorf("AC3 FAIL: secret redaction incomplete")
	}
	...
	defer func() {
		if prevReg != "" {
			os.Setenv("ANVIL_REGISTRY", prevReg)
		}
	}()
	...
	if err != nil {
		...
		for _, c := range vrOff.Checks {
			offlineLog.WriteString(fmt.Sprintf("  %s pass=%t %s\n", c.Name, c.Passed, c.Details))
		}
	...
	if !res.AC4_OfflinePass {
		return nil, fmt.Errorf("AC4 FAIL: offline verify did not pass")
	}
