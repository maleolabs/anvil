...
func TestAC1_TamperRejectedWithGuidance(t *testing.T) {
	...
	if err := TamperArtifact(pkg.ArtifactPath, tampered); err != nil {
		t.Fatalf("tamper: %v", err)
	}
	vr, _ := artifact.VerifyArtifact(tampered)
	if vr == nil || vr.Passed {
		t.Fatalf("tampered should FAIL")
	}
	_, err = VerifyBeforeExtract(tampered)
	if err == nil {
		t.Fatalf("VerifyBeforeExtract should reject tampered")
	}
	if !strings.Contains(strings.ToLower(err.Error()), "guidance") {
		t.Fatalf("guidance missing: %v", err)
	}
	if !strings.Contains(err.Error(), "abort before extract") {
		t.Fatalf("actionable guidance missing abort before extract")
	}
	// trusted must pass
	if _, err := VerifyBeforeExtract(pkg.ArtifactPath); err != nil {
		t.Fatalf("trusted should pass: %v", err)
	}
}
...
func TestAC2_PayloadIntegrityAndRepack(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("payload integrity err: %v", err)
	}
	if !pass {
		t.Fatalf("trusted payload should PASS")
	}
	...
	if pass2 {
		t.Fatalf("repacked tampered payload should be detected (FAIL)")
	}
}
...
func TestAC4_OfflineVerification(t *testing.T) {
	...
	defer func() {
		if prev != "" {
			os.Setenv("ANVIL_REGISTRY", prev)
		}
	}()
	vr, err := VerifyOffline(pkg.ArtifactPath)
	if err != nil || vr == nil || !vr.Passed {
		t.Fatalf("offline trusted should PASS: vr=%v err=%v", vr, err)
	}
	tampered := filepath.Join(tmp, "tampered-offline.tar.gz")
	_ = TamperArtifact(pkg.ArtifactPath, tampered)
	if _, err := VerifyOffline(tampered); err == nil {
...
func TestSigningFeasibilityDoc(t *testing.T) {
	...
	if !strings.Contains(doc, "Windows") || !strings.Contains(doc, "Linux") {
		t.Fatalf("signing doc incomplete")
	}
	if !strings.Contains(doc, "osslsigncode") && !strings.Contains(doc, "signtool") {
		t.Fatalf("signing doc missing tool")
	}
}
