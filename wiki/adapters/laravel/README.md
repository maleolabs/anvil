...
|---|---|
| **Init template** | `anvil init my-app --framework laravel` generates a Laravel build pipeline and framework-aware project config ([init.md](init.md)) |
...
- **Adapter binary `anvil-adapter-laravel`** — on `PATH` (required for server install/activate/rollback; see [build the binary](#build-the-adapter-binary))
- On the server / build machine: **PHP** (with the `artisan` entrypoint), **Composer**, and **Node.js/npm** (for the asset build)

> ⚠️ **Important:** the Anvil installer (`install.sh`) and release builds do **not** ship the adapter binary yet. Build it manually once (see below). This is tracked in [limitations](../limitations.md).
