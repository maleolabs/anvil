...
**Current status:** `anvil pipeline build` executes the pipeline YAML directly and does **not** invoke the adapter executable. There is no CLI command wired to the adapter's build pipeline yet — the 15-minute bound applies once that wiring lands. See [limitations](../limitations.md).
