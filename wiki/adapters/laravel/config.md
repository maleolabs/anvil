...
| `framework.laravel.cache.store` | `memcache` | ❌ `"memcache" is not a known Laravel cache store` |
| `framework.laravel.migrations.path` | `/abs/path` | ❌ `must be a relative path` |
| `framework.laravel.version` | `11` | ❌ `not valid SemVer (expected MAJOR.MINOR.PATCH)` |
| `framework.laravel.composer_flags` | `--prefer-dist; rm -rf /` | ❌ `contains shell metacharacters` |
| `framework.laravel.composer_flags` | `--no-dev` | ❌ `must not contain --no-dev` |
...
⚠️ **These keys are recognized and validated, but not yet consumed anywhere.**
