...
> **Terminology:** `anvil server release ...` commands are project-centric (for server operators). The equivalent target-centric commands (`anvil deployment install/activate/rollback`) perform the same underlying operations and honor the adapter the same way. Choose one style per workflow.
...
|---|---|---|
| `--project-id <id>` | ✅ (non-interactive) | Unique project identifier |
| `--install-root <path>` | ✅ (non-interactive) | Absolute install path on the server |
...
Build and install it once per server:
...
> The installer (`install.sh`) and release builds do not ship this binary yet — see [limitations](../limitations.md).
