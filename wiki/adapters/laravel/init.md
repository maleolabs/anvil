...
| `--framework <name>` | Framework for template generation (`laravel`, `flutter`) | (none — plain project) |
...
Fetched from the Laravel adapter's `template` command and validated through the pipeline loader (ADR-020 — the adapter owns the template). Logical content:
...
## Init without the adapter installed (fallback)

`anvil init my-app --framework laravel` needs the `anvil-adapter-laravel` binary on `PATH` to fetch the adapter-owned template. When the adapter is missing, initialization **still succeeds** (adapters are optional, ADR-009 §9.7) with the generic empty pipeline and a warning on stderr:

```text
Warning: could not generate pipeline template from adapter for framework "laravel": ...
generating the generic pipeline instead. Install the adapter and run 'anvil adapter use laravel' to regenerate.
```
