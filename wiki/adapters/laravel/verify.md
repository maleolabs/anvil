...
- Because `vendor/` must pass check #1, keep the `artifact.include: [vendor/**]` override generated by `anvil init --framework laravel` (see [init.md](init.md)). Without it, the compiled default excludes strip `vendor/` and install fails on `vendor_present`.
...
## CLI limitation: `anvil artifact verify`

`anvil artifact verify <path>` only verifies **generic integrity** — archive validity, manifest presence/content, and checksum match. It does **not** run the 8 Laravel checks. Framework verification currently happens exclusively at server install time. See [limitations](../limitations.md).
