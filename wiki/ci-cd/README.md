...
| `anvil deployment install <artifact-path>` | Local target-centric alias of `anvil server release install` — run ON the server runtime (requires a locally initialized server) |
| `anvil deployment activate <project-id> <release-id>` | Local target-centric alias of `anvil server release activate` — run ON the server runtime |
...
6. Install (server)-->  anvil server release install <project-id> <path>
                         (or the local alias: anvil deployment install <path>)
7. Activate (server)--> anvil server release activate <project-id> <release-id>
                         (or the local alias: anvil deployment activate <project-id> <release-id>)
```
...
- [Laravel standard repository](https://github.com/maleolabs/anvil-standard-laravel)
