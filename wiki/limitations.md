...
- **Status:** deferral (target state defined by ADR-020)
- **Works today:** `anvil init --framework laravel|flutter` generates `.anvil/pipelines/build.yaml` from Go data embedded in the Core binary (`internal/execution/laravel_template.go`, `flutter_template.go`); `anvil adapter list` enumerates a closed set (`engine.KnownFrameworks()` — only `laravel` and `flutter`); Laravel-specific artifact defaults (`vendor/**` include) are a Core switch
- **Does not work:** a third-party adapter binary on PATH (`anvil-adapter-rails`) is invisible to `anvil adapter list` and rejected by `anvil adapter use`; adding a framework requires a Core release; template and adapter build phases are two sources that can drift
- **Coming:** adapter-owned templates via a `template` command (TS-007-038), PATH-based discovery (TS-007-039), server build wiring (TS-007-040), build contract target/strict parity (TS-007-041) — per ADR-020
